...
async function testTokenBudgetAllocator(): Promise<IntegrationTestResult> {
  ...
  try {
    ...
    if (
      !allocation.leadAgent.model.includes('opus') &&
      !allocation.leadAgent.model.includes('claude')
    ) {
...
async function testEndToEndPipeline(): Promise<IntegrationTestResult> {
  ...
  try {
    ...
    // Step 3: Validate
    const validationResults = await executor.execute(decision.qualityGates, task, {
      decision,
      budget,
    });
    ...
    if (
      ...
      decision.qualityGates.filter((g) => g.required).length
    ) {
...
export async function runIntegrationTests(): Promise<IntegrationTestSuite> {
  ...
  console.log(
    `  ${tests[tests.length - 1].passed ? '✅' : '❌'} ${tests[tests.length - 1].testName}`,
  );
