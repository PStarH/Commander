...
describe('legacy execution guard', () => {
  it('never allows the in-process authority in production', () => {
    ...
    try {
      ...
      if (previous.v2 === undefined) delete process.env.COMMANDER_V2_MODE;
      else process.env.COMMANDER_V2_MODE = previous.v2;
      if (previous.legacy === undefined) delete process.env.COMMANDER_LEGACY_EXECUTION;
      else process.env.COMMANDER_LEGACY_EXECUTION = previous.legacy;
      if (previous.profile === undefined) delete process.env.COMMANDER_PROFILE;
      else process.env.COMMANDER_PROFILE = previous.profile;
    }
  });
