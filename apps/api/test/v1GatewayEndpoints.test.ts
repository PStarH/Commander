...
class FakeGateway implements V1KernelGateway {
  ...
  /** AUDIT F-B-13: the tenant the ROUTER passed down, not one we invented. */
  readonly submittedTenants: string[] = [];
  async submit(input: any) {
    this.submittedTenants.push(input.tenantId);
  ...
  async listEvents(runId: string, tenantId: string) {
    return (await this.getRun(runId, tenantId))
      ? ([{ id: 'event-1', runId, tenantId, type: 'run.created' }] as any[])
      : [];
  }
...
describe('V1 Gateway runs resource', () => {
  it('submits durable work asynchronously and replays an idempotent submission', async () => {
    await withGateway(new FakeGateway(), async (baseUrl) => {
      ...
      const first = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-00000001' },
  ...
  it('derives the tenant from the authenticated principal (AUDIT F-B-13)', async () => {
    const kernel = new FakeGateway();
    await withGateway(
      kernel,
      async (baseUrl) => {
        const res = await fetch(`${baseUrl}/v1/runs`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', 'idempotency-key': 'key-tenant-01' },
          body: JSON.stringify({
            goal: 'g',
            policySnapshotId: 'policy-42',
            steps: [
        ...
        // The run's tenant must equal the authenticated principal's tenant —
        // and the router must have PASSED that tenant to the kernel, not
        // echoed whatever the fixture happened to store.
        assert.equal(run.run.tenantId, 'tenant-b');
      },
      'tenant-b',
    );
    assert.deepEqual(kernel.submittedTenants, ['tenant-b']);
  });

  it('does not leak another tenant run through get (AUDIT F-B-13)', async () => {
    const kernel = new FakeGateway();
    await withGateway(
      kernel,
      async (baseUrl) => {
        const created = await fetch(`${baseUrl}/v1/runs`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', 'idempotency-key': 'key-x-01' },
          body: JSON.stringify({
            goal: 'g',
            policySnapshotId: 'policy-42',
            steps: [
        ...
        const runId = ((await created.json()) as any).run.id as string;
        // The owner can read its own run.
        const own = await fetch(`${baseUrl}/v1/runs/${runId}`);
        assert.equal(own.status, 200);
      },
      'tenant-a',
    );
    // Same kernel, different authenticated tenant: the run must be invisible.
    await withGateway(
      kernel,
      async (baseUrl) => {
        const foreign = await fetch(`${baseUrl}/v1/runs/run-key-x-01`);
        assert.equal(foreign.status, 404, 'tenant-b must not read a tenant-a run');
      },
      'tenant-b',
    );
  });
  ...
  it('rejects agent steps missing definitionVersion or providerSnapshot', async () => {
    await withGateway(new FakeGateway(), async (baseUrl) => {
      const body = {
        goal: 'x',
        policySnapshotId: 'policy-42',
        steps: [{ kind: 'agent', input: { agentId: 'a' } }],
      };
      const res = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-schema-01' },
        body: JSON.stringify(body),
      });
  ...
  it('fails closed when no shared kernel is configured', async () => {
    await withGateway(null, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-00000002' },
        body: JSON.stringify({ goal: 'do work', policySnapshotId: 'policy-42' }),
      });
  ...
  it('pauses, resumes, and cancels a durable run via the lifecycle endpoints', async () => {
    await withGateway(new FakeGateway(), async (baseUrl) => {
      ...
      const submit = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-lifecycle-01' },
        body: JSON.stringify({ goal: 'lifecycle work', policySnapshotId: 'policy-42' }),
      });
  ...
  it('returns 409 when a lifecycle transition is invalid for the current state', async () => {
    await withGateway(new FakeGateway(), async (baseUrl) => {
      ...
      const submit = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-lifecycle-02' },
        body: JSON.stringify({ goal: 'will cancel', policySnapshotId: 'policy-42' }),
      });
      const runId = ((await submit.json()) as any).run.id;
      const cancelled = await fetch(`${baseUrl}/v1/runs/${runId}/cancel`, { method: 'POST' });
      assert.equal(cancelled.status, 200);
      ...
      const submit2 = await fetch(`${baseUrl}/v1/runs`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': 'key-lifecycle-03' },
        body: JSON.stringify({ goal: 'never paused', policySnapshotId: 'policy-42' }),
      });
