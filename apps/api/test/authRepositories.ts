...
export class TestUserRepository implements UserRepository {
  ...
  async deleteUser(
    ...
    if (!membership) return { success: false, error: 'User not found' };
    this.memberships.delete(userId + ':' + tenantId);
    const hasMembership = [...this.memberships.values()].some(
      (candidate) => candidate.userId === userId,
    );
    if (!hasMembership) this.users.delete(userId);
    return { success: true };
  }
