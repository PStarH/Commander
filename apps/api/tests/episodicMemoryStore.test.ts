...
describe('EpisodicMemoryStore', () => {
  ...
  describe('write', () => {
    ...
    it('batchWrite creates multiple memories', () => {
      const results = store.batchWrite([
        { projectId: 'proj-1', title: 'Mem 1', content: 'content one with enough words', type: 'observation', tags: [], importance: 0.5 },
        { projectId: 'proj-1', title: 'Mem 2', content: 'content two with enough words', type: 'action', tags: [], importance: 0.5 },
      ]);
  ...
  describe('search', () => {
    ...
    it('filters by type', () => {
      store.write({
        projectId: 'proj-1', title: 'Obs 1', content: 'observation about system behavior', type: 'observation', tags: [], importance: 0.5,
      });
      store.write({
        projectId: 'proj-1', title: 'Dec 1', content: 'decision about system architecture', type: 'decision', tags: [], importance: 0.5,
      });
    ...
    it('filters by tags', () => {
      store.write({
        projectId: 'proj-1', title: 'Tagged', content: 'memory with important tag', type: 'observation', tags: ['important'], importance: 0.5,
      });
      store.write({
        projectId: 'proj-1', title: 'Untagged', content: 'memory without the tag', type: 'observation', tags: [], importance: 0.5,
      });
    ...
    it('respects limit', () => {
      for (let i = 0; i < 10; i++) {
        store.write({
          projectId: 'proj-1', title: `Mem ${i}`, content: `content number ${i} with enough words`, type: 'observation', tags: [], importance: 0.5,
        });
      }
    ...
    it('filters by minImportance', () => {
      store.write({
        projectId: 'proj-1', title: 'Low', content: 'low importance memory content', type: 'observation', tags: [], importance: 0.2,
      });
      store.write({
        projectId: 'proj-1', title: 'High', content: 'high importance memory content', type: 'decision', tags: [], importance: 0.9,
      });
  ...
  describe('getById', () => {
    it('retrieves memory by ID', () => {
      const memory = store.write({
        projectId: 'proj-1', title: 'Findable', content: 'this memory can be found by id', type: 'observation', tags: [], importance: 0.5,
      });
  ...
  describe('updateImportance', () => {
    it('updates importance of existing memory', () => {
      const memory = store.write({
        projectId: 'proj-1', title: 'Updatable', content: 'memory importance can be updated', type: 'observation', tags: [], importance: 0.5,
      });
  ...
  describe('delete', () => {
    it('deletes a memory', () => {
      const memory = store.write({
        projectId: 'proj-1', title: 'Deletable', content: 'this memory will be deleted', type: 'observation', tags: [], importance: 0.5,
      });
  ...
  describe('getStats', () => {
    it('returns correct statistics', () => {
      store.write({ projectId: 'proj-1', title: 'Obs', content: 'observation content', type: 'observation', tags: [], importance: 0.5 });
      store.write({ projectId: 'proj-1', title: 'Dec', content: 'decision content', type: 'decision', tags: [], importance: 0.8 });
