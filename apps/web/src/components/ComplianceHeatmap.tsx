    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
      ...
      <div className="card" style={{ padding: '16px 18px' }}>
        <div className="section-head" style={{ marginBottom: '12px' }}>
          ...
          <span className="section-tag">
            {nist.alignmentPercentage}%
          </span>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {(Object.entries(nist.functionCoverage) as [string, typeof nist.functionCoverage['GOVERN']][])
            .map(([func, entry]) => {
              const pct = entry.coveragePercentage;
              const color = pct >= 90 ? 'var(--accent-green)' : pct >= 70 ? 'var(--accent-blue)' : 'var(--accent-amber)';
              return (
                <div key={func}>
                  ...
                  <div style={{ position: 'relative', height: '6px', borderRadius: '3px', background: 'var(--border-subtle)', overflow: 'hidden' }}>
                    <div style={{
                      position: 'absolute',
                      inset: 0,
                      borderRadius: '3px',
