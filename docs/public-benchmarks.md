...
|------|:---:|
| 注册工具总数 | 19 |
| 工具分类数 | 6（web, filesystem, code, memory, multimodal, development） |
| 含安全标识的工具数 | 5+（isConcurrencySafe, isReadOnly, timeout） |
| 工具 Schema 完备率 | 100% |
...
BFCL                Built-in(19ツー 通过LLM    通过LLM      通过LLM    通过LLM    通过LLM
                    ル)
Tool Count          19              ~8        8            11         20+        70+
Error Classification 5/5 correct    Via       Via classifier Built-in  Via hooks  Via callbacks
                         ExecPolicy  ExecPolicy
Context Compaction  4-layer         1-layer   4-layer      2-layer    2-layer    2-layer
```
...
- **工具数量与分类**：19 个内置工具覆盖 6 个类别，包括多模态工具（vision_analyze、pdf_extract、screenshot_capture）
