...
**状态：IN PROGRESS（Phase 3 审计发现 3 项 High 未闭环，2026-07-16 复审驳回 ACCEPTED）**
**范围：Phase 1 Spec → Phase 2 Build → Phase 3 Review & Audit**

> 复审驳回理由（证据见分支审计报告）：
> - §1/§5 标记与实现不符：LLM 出口在 `workerRuntimeAdapter.ts` 被 `void effectBroker` 丢弃，未经 admit→execute；kernel 白名单/配额引擎存在但 `admit()` 从不调用（孤儿）。
> - §8 补偿消费失败无 DLQ/max_attempts，毒消息 60s 无限重试。
> 上述项闭环前，§1、§5、§8 相关勾选不成立。
...
新增 `packages/operations/src/compensationConsumer.ts`（独立于 `OutboxPublisher`，专消费 `commander.compensation` topic）：
...
- [~] §1 EffectBroker 为唯一对外 IO 出口；工具/连接器/补偿经 admit→execute。**LLM 出口未闭环**（`workerRuntimeAdapter.ts` void 丢弃 broker）。
  — 证据：`toolStepExecutor.ts:83` 无 broker 即抛 `EFFECT_BROKER_UNAVAILABLE`；`connectorStepExecutor.ts:104` 同；`workerRuntimeAdapter.ts:20` LLM executor 持有 `effectBroker` 引用；`compensationConsumer.ts:148-163` 补偿经 `broker.admit().executeAdmitted()`。
...
- [~] §5 操作白名单、速率限制、租户配额三层策略引擎实现；**但 `admit()` 从不调用它们**（引擎孤立，实际授权仅依赖注入的 PolicyEvaluator）。
...
- [~] §8 补偿消费者实现；补偿事件经 EffectBroker admit→execute 执行。**失败无 DLQ/max_attempts，毒消息 60s 无限重试**（§8.3 违反）。
  — 证据：`packages/operations/src/compensationConsumer.ts` 存在；`consumeCompensationBatch()` 第 148 行 `broker.admit()`、第 163 行 `broker.executeAdmitted()`；`kernel/ws2-acceptance.test.ts` §8 "claimOutboxByTopic" 2/2 通过。
