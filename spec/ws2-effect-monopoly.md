...
**状态：PARTIAL（§5/§8 已闭环；§8 claim 过滤已对称；§1 LLM：call-time mint + provider wrap + 默认允许 `llm.*`（policy + allowlist seed）+ 工具类仍 deny-default；遗留：API/Gateway 路径 LLM、多 isolate invoke registry）**
...
- [~] §1 EffectBroker 为唯一对外 IO 出口；工具/连接器/补偿经 admit→execute。**LLM（worker agent）：call-time mint + contentHash 绑定 + wrap + step ALS + 默认允许 `llm.*` + 生产拒建**；工具类仍需 allowlist/PolicyEvaluator；非 worker 入口未强制。
