...
**状态：PARTIAL（§5/§8 已于 2026-07-16 闭环并有验收测试；唯一遗留 §1 LLM 出口为显式 deferred）**
...
> - §8 补偿失败现调 `retryOutbox`（错误记录+立即退避），毒消息由 `sweepOutboxDlq`（kernel-ops timer 每周期运行）按 max_attempts 落 DLQ；consumer 单测四例证明重试有界。
> - §1 LLM 出口仍为 deferred：wiring contract 已建立（`workerRuntimeAdapter.effectBroker`），provider 调用拦截需在 core 运行时设计 token 铸造链路，为独立后续工作。
