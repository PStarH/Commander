...
**状态：PARTIAL（§3–§9 worker broker 主路径已闭环；§1 LLM worker：call-time mint + wrap + 默认允许 `llm.*` + C-α 租户 registry/worker affinity 已落地见 `spec/ws2-llm-multi-isolate.md`；§1 tool/connector worker 路径 L3-03a ENFORCED via `effectGate`；遗留：API/Gateway LLM、Gateway tool token 颁发、C-β 调度标签）**
...
- [~] §1 EffectBroker 为唯一对外 IO 出口；工具/连接器/补偿经 admit→execute。**LLM（worker agent）：call-time mint + contentHash 绑定 + wrap + step ALS + 默认允许 `llm.*` + 生产拒建**。**Tool/connector（worker）：L3-03a 生产默认强制 broker（省略 `hasExternalEffects` 不可旁路）；`localOnly` 豁免仅显式内部工具；deny-default + allowlist ENFORCED 有端到端验收**。非 worker / Gateway catalog 强制仍为 L3-03b follow-up。
  — 证据：`effectGate.ts` + `toolStepExecutor.ts` / `connectorStepExecutor.ts` / `workerRuntimeAdapter.ts`；`l3-03a-effect-tool-monopoly.test.ts` 12/12；`llmBrokerBridge.ts`；`bootstrap.policy.test.ts`。
