...
| # | 条件 | 判定 |
|---|------|------|
| 1 | 产品 memory 写双权威收敛为单一首选写 API | **ENFORCED** — `writeProductMemory` → `MemoryStore.write` → `MemoryService.store` |
...
| ThreeLayerMemory | product | **product** | Working / routing；durable 出站经 MemoryStore |
...
| `UnifiedMemory.remember` / `ProjectMemoryStoreAdapter.append` / namespaced HTTP | direct `store.write` | **must** call `writeProductMemory` | via store |
