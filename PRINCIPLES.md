...
| Memory system         | **21** (methodology-locked, was census 19) | `UnifiedMemory` (`core/src/memory/unifiedMemory.ts:167`) over `ThreeLayerMemory`                                                                                    | `apps/api` runs a **parallel** memory system that bypasses `UnifiedMemory` (`memoryStore.ts:54`, `memoryIndexManager.ts:64`, `namespacedMemoryStore.ts:151`); `MemoryCurator` is **two different classes, both wired** (`memory/curator.ts:83` + `memory/memoryCurator.ts:27`); `MemorySystem` is `@deprecated` with no callers                                                                                                                                         |
