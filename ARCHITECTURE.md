...
- **V2 — Enterprise Gateway SKU.** The plane-separated target: `@commander/contracts`
  (types) → `@commander/kernel` (durable Postgres authority + `src/ops` always-on binary
  for reclaim/timer/outbox as `kernel-ops`) → `@commander/worker-plane` (execution) +
  ...
  arch-guard bans it. Ops = kernel-ops. EffectBroker-backed compensation drain is a **known
  ownership gap**; L4-B follow-up may add deploy unit `@commander/adapter-ops` (**not** a
  rename of `@commander/operations`, **not** on master today). Partially built; durable `/v1`
  kernel defaults ON in production / V2 mode / when a DSN is set (`isCommanderKernelEnabled`).
  **Alpha** for multi-tenant enterprise use.
...
| `packages/kernel`         | Postgres durable authority + **kernel-ops** (`src/ops`: reclaim, timer, outbox). Hosts `consumeCompensationBatch` **library** only.            | WIRED for reclaim/timer/outbox; `/v1`-only authority                  |
| `packages/worker-plane`   | Poll/claim/execute; invokes core `AgentRuntime` via sanctioned bridge.                                                                         | WIRED; core bridge allowlisted                                        |
| `packages/effect-broker`  | Capability PEP for external effects; fail-closed.                                                                                              | EXISTS; worker-plane is primary host                                  |
...
| `packages/adapter-ops`    | **L4-B follow-up** deploy unit (EffectBroker compensation/reconcile). **Not on master**; not a V2 plane; not a resurrected operations package. | ABSENT (follow-up) — compensation production drain **unwired**        |
