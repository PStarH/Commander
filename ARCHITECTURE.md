...
- **V2 — Enterprise Gateway SKU.** The plane-separated target: `@commander/contracts`
  ...
  `@commander/operations` was folded into kernel-ops in WS1 and must not reappear as a
  plane package. Adapter compensation/reconcile drain (when EffectBroker + action-adapters
  are required) lives in the deploy unit `@commander/adapter-ops` — **not** a fifth plane.
...
| ~~`packages/operations`~~ | **Deleted in WS1.** Do not reintroduce as a plane; arch-guard bans `@commander/operations`.                                                       | ABSENT + import-banned                                                |
| `packages/adapter-ops`    | Deploy unit only: EffectBroker-backed compensation/reconcile drain. **Not a V2 plane.**                                                           | PARTIAL — fail-closed registry PEP                                    |
