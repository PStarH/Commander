...
export class CampaignFaultControlHandler {
  ...
  async handle(input: {
    ...
    const capabilityLifetimeMs = Date.parse(command.expiresAt) - this.clock().getTime();
    if (capabilityLifetimeMs <= 0) {
      await this.audit(command, 'fault_control.rejected', 'high', {
        code: 'FAULT_CONTROL_CAPABILITY_EXPIRED',
      });
      return { accepted: false, code: 'FAULT_CONTROL_CAPABILITY_EXPIRED' };
    }
    const expiresFirst = capabilityLifetimeMs <= requestedTimeoutMs;
    const timeoutMs = Math.min(requestedTimeoutMs, capabilityLifetimeMs);
