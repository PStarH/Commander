...
describe('Parameter Controller — Sampling Params', () => {
  it('code_generation uses low temperature', () => {
    ...
    assert.ok(
      ...
      `code_generation temp should be <= 0.3, got ${params.temperature}`,
    );
  ...
  it('creative uses high temperature', () => {
    ...
    assert.ok(
      ...
      `creative temp should be >= 0.7, got ${params.temperature}`,
    );
  });
  ...
  it('tool_calling uses very low temperature', () => {
    ...
    assert.ok(
      ...
      `tool_calling temp should be <= 0.1, got ${params.temperature}`,
    );
  });
  ...
  it('low confidence constrains temperature', () => {
    ...
    assert.ok(
      ...
      `Low confidence should clamp temp, got ${params.temperature}`,
    );
  });
});
...
describe('Parameter Controller — Multi-Turn Strategy', () => {
  it('first retry increases temperature slightly', () => {
    ...
    assert.ok(
      params1.temperature >= params0.temperature,
      `Retry should increase temp (${params0.temp} -> ${params1.temp})`,
    );
  });
  ...
  it('third retry falls back to low temperature', () => {
    ...
    assert.ok(
      params2.temperature <= 0.2,
      `Third retry should have low temp, got ${params2.temperature}`,
    );
  });
});
