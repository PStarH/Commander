...
import { installProcessCrashHandlers, isShuttingDown, resetCrashHandlersForTesting } from '../../src/runtime/processCrashSafety';
...
describe('ProcessCrashSafety — reversibility tier 1.1', () => {
  ...
  it('records DLQ entry on uncaughtException', () => {
    const deps = {
      dlq, leaseManager: lm, activeRunIds: () => activeRuns,
    ...
    const crashEntries = entries.filter(e => e.tags.includes('crash') && e.tags.includes('uncaughtException') && e.runId === 'run-1');
  ...
  it('records DLQ entry on unhandledRejection', () => {
    ...
    const entries = dlq.readEntries('execution', 100);
    const crashEntries = entries.filter(e => e.runId === 'run-2' && e.tags.includes('unhandledRejection'));
  ...
  it('releases lease and aborts scheduler for crashed run', () => {
    ...
    assert.strictEqual(validateResult, null, 'lease should be released after crash (validate returns null)');
  });
  ...
  it('skips reentry — only one shutdown sequence runs', () => {
    ...
    process.exit = (() => { shutdownCount++; }) as never;
