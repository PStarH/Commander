...
describe('recordPromptCacheSavings', () => {
  ...
  it('is a no-op when cacheReadTokens === 0', () => {
    metrics.recordPromptCacheSavings(
      usage(10_000, 0),
      'anthropic',
      'claude-3-5-sonnet',
      'tenant-a',
  ...
  it('emits the expected savings on claude-3-5-sonnet with 10K cached tokens', () => {
    ...
    expect(metrics.getCounterTotal('prompt_cache_dollars_uncached_equivalent_total')).toBeCloseTo(
      0.03,
      6,
    );
  ...
  it('falls through to FALLBACK_PRICING for unknown models on a known provider', () => {
    ...
    expect(metrics.getCounterTotal('prompt_cache_cost_saved_usd_total')).toBeCloseTo(0.01, 6);
  });
...
describe('recordPromptPrefixCache (public API — unchanged)', () => {
  ...
  it('still increments prompt_prefix_cache_total with outcome=hit|miss', () => {
    ...
    // hits
    expect(
      metrics.getCounter('prompt_prefix_cache_total', [{ name: 'outcome', value: 'hit' }]),
    ).toBe(2);
    // misses
    expect(
      metrics.getCounter('prompt_prefix_cache_total', [{ name: 'outcome', value: 'miss' }]),
    ).toBe(1);
  });
...
describe('CostDashboard surfaces the new fields', () => {
  ...
  beforeEach(() => {
    ...
    metrics.incrementCounter('prompt_cache_tokens_read_total', 'seed', 12_345, [
      { name: 'provider', value: 'anthropic' },
    ]);
    metrics.incrementCounter('prompt_cache_dollars_uncached_equivalent_total', 'seed', 0.045, [
      { name: 'provider', value: 'anthropic' },
    ]);
    metrics.incrementCounter('prompt_cache_cost_saved_usd_total', 'seed', 0.0405, [
      { name: 'provider', value: 'anthropic' },
    ]);
...
describe('CostModel.getSavingsForCachedReads (pure)', () => {
  ...
  it('matches the documented 10x ratio for claude-3-5-sonnet at 10K tokens', () => {
    ...
    expect(r.dollarsUncachedEquivalent).toBeCloseTo(0.03, 6);
