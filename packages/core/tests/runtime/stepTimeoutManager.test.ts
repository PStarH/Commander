...
describe('StepTimeoutManager', () => {
  ...
  it('rejects with StepTimeoutError when promise exceeds timeout', async () => {
    ...
    await expect(mgr.wrap(slow, { stepId: 's2', timeoutMs: 30 })).rejects.toBeInstanceOf(StepTimeoutError);
  });
  ...
  it('preserves original error when promise rejects for non-timeout reason', async () => {
    ...
    await expect(mgr.wrap(failing, { stepId: 's5', timeoutMs: 1000 })).rejects.toThrow('original failure');
  });
});
