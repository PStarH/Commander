...
describe('compactToolDef', () => {
  it('strips descriptions from properties', () => {
    const def = makeToolDef('test_tool', 'A test tool', {
      path: { type: 'string', description: 'The file path to read' },
      pattern: { type: 'string', description: 'Search pattern' },
    }, ['path']);
    const compact = compactToolDef(def);
    const props = (compact.inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
  ...
  it('strips examples when configured', () => {
    const def = makeToolDef('test_tool', 'Test', {
      path: { type: 'string' },
    }, ['path'], [{ name: 'test_tool', arguments: { path: '/foo' } }]);
    const compact = compactToolDef(def, { ...new ProgrammaticToolFormatter().getConfig(), stripExamples: true });
  ...
  it('keeps examples when stripExamples is false', () => {
    ...
    const compact = compactToolDef(def, { ...new ProgrammaticToolFormatter().getConfig(), stripExamples: false });
  ...
  it('keeps full schema for tools in keepFullSchema', () => {
    ...
    const config: CompactToolConfig = { ...new ProgrammaticToolFormatter().getConfig(), keepFullSchema: ['git'] };
  ...
  it('returns original when disabled', () => {
    ...
    const config: CompactToolConfig = { ...new ProgrammaticToolFormatter().getConfig(), enabled: false };
    const compact = compactToolDef(def, config);
    const props = (compact.inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
...
describe('compactToolDefs', () => {
  it('compacts all tools in the array', () => {
    ...
    for (const c of compacted) {
      const props = (c.inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
...
describe('buildCompactToolListing', () => {
  it('produces compact parameter-style listing', () => {
    ...
    tools.set('file_read', makeTool('file_read', makeToolDef('file_read', 'Read file contents', {
      path: { type: 'string', description: 'File path' },
    }, ['path'])));
    tools.set('shell_execute', makeTool('shell_execute', makeToolDef('shell_execute', 'Run shell command', {
      cmd: { type: 'string', description: 'Command' },
      cwd: { type: 'string', description: 'Working dir' },
    })));

    const listing = buildCompactToolListing(tools);
    assert.ok(listing.includes('file_read(path: string)'), `Expected 'file_read(path: string)' in:\n${listing}`);
  ...
  it('handles enum parameters', () => {
    const tools = new Map<string, Tool>();
    tools.set('verify', makeTool('verify', makeToolDef('verify', 'Verify code', {
      checks: {
        type: 'array',
  ...
  it('handles enum property as union type', () => {
    const tools = new Map<string, Tool>();
    tools.set('mode', makeTool('mode', makeToolDef('mode', 'Set mode', {
  ...
  it('handles tools with no parameters', () => {
    ...
    const listing = buildCompactToolListing(tools);
    assert.ok(listing.includes('ping()') || listing.includes('ping —'), `Expected ping listing in:\n${listing}`);
  });
  ...
  it('falls back to name:description format when compactListing is false', () => {
    ...
    const config: CompactToolConfig = { ...new ProgrammaticToolFormatter().getConfig(), compactListing: false };
    const listing = buildCompactToolListing(tools, config);
    assert.ok(listing.includes('- file_read: Read file contents'), `Expected old format, got:\n${listing}`);
  });
});
...
describe('formatCompactToolCall', () => {
  ...
  it('truncates long outputs', () => {
    ...
    const config: CompactToolConfig = { ...new ProgrammaticToolFormatter().getConfig(), maxToolCallChars: 100 };
  ...
  it('truncates long argument values', () => {
    const call: ToolCall = { id: 'call_1', name: 'file_write', arguments: { path: '/foo', content: 'x'.repeat(200) } };
  ...
  it('falls back to verbose format when compactToolCalls is false', () => {
    const call: ToolCall = { id: 'call_1', name: 'file_read', arguments: { path: '/foo' } };
    const config: CompactToolConfig = { ...new ProgrammaticToolFormatter().getConfig(), compactToolCalls: false };
    const formatted = formatCompactToolCall(call, 'output', config);
...
describe('estimateCompactSavings', () => {
  it('estimates positive savings for tools with descriptions', () => {
    const tools: ToolDefinition[] = [
      makeToolDef('file_read', 'Read file', {
      ...
      makeToolDef('file_write', 'Write file', {
    ...
    assert.ok(savings.schemaSavings > 0, `Expected positive schemaSavings, got ${savings.schemaSavings}`);
...
describe('minifyToolDef', () => {
  it('shortens aliased parameter names', () => {
    const def = makeToolDef('shell_tool', 'Run shell command', {
      command: { type: 'string', description: 'Shell command' },
      pattern: { type: 'string', description: 'Pattern' },
    }, ['command']);

    const minified = minifyToolDef(def);
    const props = (minified.inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
  ...
  it('restores original parameter names', () => {
    ...
    const props = (restored.inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
    assert.ok('command' in props);
    assert.ok('pattern' in props);
  ...
  it('minifies an array of tool definitions', () => {
    ...
    const p1 = (minified[0].inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
    const p2 = (minified[1].inputSchema as Record<string, unknown>).properties as Record<string, unknown>;
    assert.ok('cmd' in p1);
    assert.ok('pat' in p2);
  });
