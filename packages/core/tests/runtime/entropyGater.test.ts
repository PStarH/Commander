...
describe('EntropyGater - isConfidentResponse', () => {
  ...
  it('returns true for confident final answers', () => {
    expect(
      isConfidentResponse({
        content:
          'Here is the complete analysis. The result shows that the system performs well under load. In summary, the architecture is sound.',
        finishReason: 'stop',
      }),
    ).toBe(true);
  });
  ...
  it('returns true for long definitive content', () => {
    expect(
      isConfidentResponse({
        content:
          'Based on the analysis, the optimal configuration is to use 8 workers with a batch size of 64. This maximizes throughput while minimizing latency. The data clearly supports this conclusion across all tested scenarios.',
        finishReason: 'stop',
      }),
    ).toBe(true);
  });
...
describe('EntropyGater - hasInformationGain', () => {
  ...
  it('returns true for state-mutating tools', () => {
    expect(hasInformationGain([{ name: 'shell_execute', arguments: {} }], [])).toBe(true);
    expect(hasInformationGain([{ name: 'file_write', arguments: {} }], [])).toBe(true);
  });

  it('returns true when no recent results exist', () => {
    expect(hasInformationGain([{ name: 'web_search', arguments: { query: 'test' } }], [])).toBe(
      true,
    );
  });
