...
} from '../../src/tools/conversationSearchTool';
  ...
  describe('end-to-end: create → search → verify', () => {
    it('finds conversations matching full-text query', async () => {
      ...
      await store.addTurn({
        ...
        role: 'user',
        content:
          'What database should we use for the new project? I think PostgreSQL would be best.',
      });
      await store.addTurn({
        sessionId: session.id,
        role: 'assistant',
        content:
          'PostgreSQL is a great choice. We can use FTS5 full-text search for conversation history and implement proper indexing strategies.',
      });
      await store.addTurn({
        sessionId: session.id,
        role: 'user',
      ...
      assert.ok(
        result.includes('Database schema design discussion'),
        'Result should include the session goal',
      );
      assert.ok(
        result.includes('100%') || result.includes('Relevance:'),
        'Result should show relevance',
      );
    ...
    it('searches across multiple sessions and returns best matches', async () => {
      ...
      await store.addTurn({
        sessionId: s1.id,
        role: 'assistant',
        content: 'Run apt-get install nginx',
      });
      await store.endSession(s1.id);

      // Session 2: target topic
      const s2 = await store.startSession({
        projectId: 'test-project',
        goal: 'React component design',
      });
      await store.addTurn({
        sessionId: s2.id,
        role: 'user',
        content: 'How to build a React search component with TypeScript?',
      });
      await store.addTurn({
        sessionId: s2.id,
        role: 'assistant',
        content: 'We can create a SearchBar component using React hooks and TypeScript generics.',
      });
      ...
      await store.addTurn({
        sessionId: s3.id,
        role: 'user',
        content: 'Should we write the API in TypeScript or Rust?',
      });
      await store.addTurn({
        sessionId: s3.id,
        role: 'assistant',
        content: 'Rust with Actix-web gives better performance for the API layer.',
      });
      ...
      assert.ok(
        result.includes('React component design'),
        'Should find the TypeScript-related session',
      );
    ...
    it('isolates search by projectId', async () => {
      ...
      await store.addTurn({
        sessionId: sA.id,
        role: 'user',
        content: 'This is about project A and Kubernetes',
      });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('handles special characters in query', async () => {
      ...
      await store.addTurn({
        sessionId: session.id,
        role: 'user',
        content: 'The C++ code uses lambda functions',
      });
      await store.endSession(session.id);
    ...
    it('filters by sinceDays parameter', async () => {
      ...
      assert.ok(
        resultWide.includes('Recent discussion'),
        'sinceDays=36500 should include recent sessions',
      );
      ...
      assert.ok(storeResultsAll.length >= 1, 'Without since filter, the session should be found');
    });
    ...
    it('filters by minImportance parameter', async () => {
      ...
      await store.addTurn({
        ...
        role: 'user',
        content:
          'I decided to use TypeScript because I learned it reduces bugs in api/src/main.ts.',
      });
      ...
      assert.ok(
        resultHigh.includes('decided to use TypeScript'),
        'minImportance=0.8 should keep high-importance turn (score ~1.0)',
      );
      assert.ok(
        resultHigh.match(/Found \d+ conversation/),
      ...
      assert.ok(
        resultMedium.includes('No conversations'),
      ...
      assert.ok(
        resultMediumAll.includes('Fixed the error'),
      ...
      assert.ok(
        resultLow.includes('No conversations'),
    ...
    it('returns results sorted by relevance score descending', async () => {
      ...
      const sA = await store.startSession({
        projectId: 'test-sort',
        goal: 'Session A — Deep TypeScript discussion',
      });
      await store.addTurn({
        sessionId: sA.id,
        role: 'user',
        content: 'I decided to use TypeScript because I learned it reduces bugs.',
      }); // ~1.0
      await store.addTurn({
        sessionId: sA.id,
        role: 'user',
        content: 'The key takeaway is that TypeScript catches errors early.',
      }); // ~0.75
      await store.addTurn({
        sessionId: sA.id,
        role: 'assistant',
        content: 'We fixed a bug in main.ts because TypeScript found the issue.',
      }); // ~0.85
      await store.addTurn({
        sessionId: sA.id,
        role: 'user',
        content: 'I realized TypeScript is a key advantage for our project.',
      }); // ~0.75
      await store.endSession(sA.id);

      // Session B — medium relevance: 2 medium-importance turns matching "TypeScript"
      const sB = await store.startSession({
        projectId: 'test-sort',
        goal: 'Session B — Brief TypeScript mention',
      });
      await store.addTurn({
        sessionId: sB.id,
        role: 'user',
      ...
      await store.addTurn({
        sessionId: sB.id,
        role: 'assistant',
        content: 'TypeScript offers better IDE support for our team.',
      }); // ~0.6
      await store.endSession(sB.id);

      // Session C — lowest relevance: 1 short low-importance turn matching "TypeScript"
      const sC = await store.startSession({
        projectId: 'test-sort',
        goal: 'Session C — Casual TypeScript mention',
      });
      ...
      assert.ok(
        goals[0].includes('Session A'),
        'Top result should be Session A (highest relevance)',
      );
      assert.ok(
        goals[1].includes('Session B'),
        'Middle result should be Session B (medium relevance)',
      );
      assert.ok(
        goals[2].includes('Session C'),
        'Last result should be Session C (lowest relevance)',
      );
    ...
    it('respects the limit parameter', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const s = await store.startSession({
          ...
          goal: `Limit test Session ${i}`,
        });
        await store.addTurn({
          sessionId: s.id,
          role: 'user',
          content: `Discussion about index strategies part ${i}`,
        });
  ...
  describe('searchConversationsCLI', () => {
    it('supports JSON output format', async () => {
      ...
      await store.addTurn({
        sessionId: session.id,
        role: 'user',
        content: 'This is a test about JSON output',
      });
      await store.endSession(session.id);
