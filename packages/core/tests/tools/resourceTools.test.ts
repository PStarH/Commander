...
describe('STRAP resource tools', () => {
  it('createAllTools exposes only consolidated resource tools and singletons', () => {
    ...
    const legacy = [
      'file_read', 'file_write', 'file_edit', 'file_search', 'file_list', 'glob',
      'web_search', 'web_fetch', 'browser_search', 'browser_fetch',
      'memory_store', 'memory_recall', 'memory_list',
      'code_search', 'refine_code', 'fix_code',
      'python_execute', 'shell_execute', 'execute_script',
      'vision_analyze', 'screenshot_capture', 'pdf_extract',
      'checkpoint_save', 'checkpoint_rewind', 'checkpoint_list', 'checkpoint_collapse',
      'handoff_check',
      'request_human_input', 'request_tool',
    ];
  ...
  it('file resource tool has action discriminator and required action param', () => {
    ...
    expect(schema.properties.action).toEqual(
      expect.objectContaining({ type: 'string', enum: expect.arrayContaining(['read', 'write', 'edit', 'search', 'list', 'glob']) }),
    );
  });
  ...
  it('exec resource tool has python, shell, and script actions', () => {
    ...
    const schema = tool.definition.inputSchema as { properties: Record<string, unknown>; required: string[] };
    expect(schema.required).toContain('action');
    expect((schema.properties.action as { enum: string[] }).enum).toEqual(expect.arrayContaining(['python', 'shell', 'script']));
  });

  it('media resource tool has analyze_image, screenshot, extract_pdf actions', () => {
    const tool = new MediaResourceTool();
    const schema = tool.definition.inputSchema as { properties: Record<string, unknown>; required: string[] };
    expect((schema.properties.action as { enum: string[] }).enum).toEqual(expect.arrayContaining(['analyze_image', 'screenshot', 'extract_pdf']));
  });

  it('system resource tool has human_input and tool_schema actions', () => {
    const tool = new SystemResourceTool();
    const schema = tool.definition.inputSchema as { properties: Record<string, unknown>; required: string[] };
    expect((schema.properties.action as { enum: string[] }).enum).toEqual(expect.arrayContaining(['human_input', 'tool_schema']));
  });
