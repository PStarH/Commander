...
describe('EffortScaler', () => {
  ...
  it('should select canonical topologies for different effort levels', () => {
    // D3.2 consolidation (src/ultimate/types.ts:79-84): the legacy topology names
    // were merged into 5 canonical patterns, so MODERATE is DISPATCH (formerly
    // PARALLEL) and COMPLEX / DEEP_RESEARCH are both ORCHESTRATOR (formerly
    // HIERARCHICAL / HYBRID). EFFORT_RULES.recommendedTopology carries the
    // canonical name per level (src/ultimate/effortScaler.ts:18,30,42,54).
    assert.strictEqual(selectTopologyForEffort('SIMPLE'), 'SINGLE');
    assert.strictEqual(selectTopologyForEffort('MODERATE'), 'DISPATCH');
    assert.strictEqual(selectTopologyForEffort('COMPLEX'), 'ORCHESTRATOR');
    assert.strictEqual(selectTopologyForEffort('DEEP_RESEARCH'), 'ORCHESTRATOR');
  });

  it('should select chained topology for tightly coupled tasks', () => {
    // Coupling > 0.7 short-circuits to the canonical CHAIN pattern
    // (src/ultimate/effortScaler.ts:99-101); SEQUENTIAL is its legacy alias
    // (src/ultimate/types.ts:114).
    const topology = selectTopologyForEffort('MODERATE', {
      parallelismWidth: 1,
      criticalPathDepth: 5,
      interSubtaskCoupling: 0.9,
    });
    assert.strictEqual(topology, 'CHAIN');
  });
});
