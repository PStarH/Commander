...
describe('HallucinationDetector', () => {
  ...
  it('returns risk score between 0 and 1', () => {
    const report = detector.analyze(
      'test',
      'I am 100% certain and without doubt. A recent study by Dr. Jones found 99% improvement.',
    );
  ...
  describe('Entailment failure detection', () => {
    ...
    it('does not flag output grounded in input', () => {
      ...
      const output =
        'The Commander multi-agent architecture uses a lead agent that coordinates with subagents. The key components include task decomposition, quality gates, and consensus verification.';
      const report = detector.analyze(input, output);
  ...
  describe('Edge cases', () => {
    ...
    it('handles Chinese content', () => {
      const report = detector.analyze('这个系统怎么样？', '毫无疑问，这个系统是百分之百可靠的。');
      assert.ok(report.signals.some((s) => s.type === 'overconfidence'));
    });
