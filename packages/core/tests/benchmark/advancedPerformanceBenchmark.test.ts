  ...
  const msgs: LLMMessage[] = [
    {
      role: 'system',
      content:
        'You are a senior software engineer helping debug a Node.js/Express authentication system.',
    },
  ];
...
function makeRealisticToolCalls(count: number): ToolCall[] {
  const patterns = [
    ...
    {
      name: 'file_write',
      args: (i: number) => ({ path: `src/middleware/auth.ts`, content: `// Updated line ${i}` }),
    },
    {
      name: 'shell_execute',
      args: (i: number) => ({ command: `npx vitest run tests/auth.test.ts --reporter=verbose` }),
    },
    { name: 'git_diff', args: (i: number) => ({ path: `src/middleware/auth.ts` }) },
    { name: 'file_read', args: (i: number) => ({ path: `tests/auth.test.ts`, line: i * 8 }) },
    {
      name: 'web_search',
      args: (i: number) => ({ query: `Express.js middleware best practices ${i}` }),
    },
    {
      name: 'file_write',
      args: (i: number) => ({ path: `tests/auth.test.ts`, content: `// Test ${i}` }),
    },
  ];
...
function makeRealisticTools(): Map<string, Tool> {
  const defs: Array<{ name: string; desc: string; category: string; safe: boolean }> = [
    {
      name: 'file_read',
      desc: 'Read file contents with optional line range',
      category: 'filesystem',
      safe: true,
    },
    {
      name: 'file_write',
      desc: 'Write content to file, creates directories if needed',
      category: 'filesystem',
      safe: false,
    },
    {
    ...
    {
      name: 'web_search',
      desc: 'Search web for documentation and examples',
      category: 'web',
      safe: true,
    },
  ];
...
describe('Advanced Performance Benchmarks', () => {
  ...
  it('context compactor throughput (realistic messages)', async () => {
    ...
    const result: BenchmarkResult = {
      ...
      metrics: {
        ...
        results: results
          .map((r) => `${r.turns}turns:${r.p99_ms}ms/${r.messages_compacted}left`)
          .join(', '),
        target_p99_ms: 50,
      },
  ...
  it('semantic cache lookup latency (realistic queries)', async () => {
    ...
    for (const req of requests) {
      await cache.store(req, {
        ...
        choices: [
          {
            message: { role: 'assistant', content: 'Detailed response with code examples...' },
            finish_reason: 'stop',
            index: 0,
          },
        ],
  ...
  it('deliberation pipeline end-to-end (realistic tasks)', () => {
    const tasks = [
      ...
      {
        name: 'docs_update',
        goal: 'Update the API documentation to reflect the new authentication endpoints and error response formats',
      },
      {
        name: 'dep_upgrade',
        goal: 'Upgrade from Express 4 to Express 5 and fix all breaking changes in the routing and middleware chain',
      },
    ];
    ...
    const result: BenchmarkResult = {
      ...
      metrics: {
        ...
        classifications: plans
          .map((p) => `${p.task}→${p.type}/${p.topology}/${p.agents}agents`)
          .join(', '),
        target_p99_us: 500,
      },
      timestamp: new Date().toISOString(),
