...
import { AGENT_CARD_WELL_KNOWN_PATH, A2A_PROTOCOL_VERSION, A2A_VERSION_HEADER } from '../src/mcp/a2aCompliance';
...
describe('A2AClient', () => {
  ...
  it('fetches Agent Card from well-known endpoint', async () => {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      res.writeHead(req.url === AGENT_CARD_WELL_KNOWN_PATH ? 200 : 404, { 'Content-Type': 'application/json' });
  ...
  it('constructs proper JSON-RPC request for sendMessage', async () => {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      req.on('end', () => {
        ...
        res.end(JSON.stringify({
          jsonrpc: '2.0', id: 1,
          result: { id: 'a2a_test_1', contextId: 'ctx_1', status: { state: 'SUBMITTED', timestamp: new Date().toISOString() } },
        }));
      });
    });
    await new Promise<void>(resolve => server.listen(0, '127.0.0.1', resolve));
    ...
    const client = new A2AClient(`http://127.0.0.1:${port}`);
    await client.sendMessage({ messageId: 'msg-1', role: 'user', parts: [{ type: 'text', text: 'Do something' }] });
