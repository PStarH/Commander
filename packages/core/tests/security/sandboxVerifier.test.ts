...
} from '../../src/security/sandboxVerifier';
...
describe('SandboxVerifier', () => {
  ...
  it('runs verification and returns a report', async () => {
  ...
  it('quickCheck returns boolean', async () => {
    const passed = await verifier.quickCheck();
    expect(typeof passed).toBe('boolean');
  }, SANDBOX_TIMEOUT);

  it('evidence entries have required fields', async () => {
    const report = await verifier.verify();
  ...
  it('results map covers all areas', async () => {
    const report = await verifier.verify();
  ...
  it('includes recommendations for noop sandbox', async () => {
    const report = await verifier.verify();
