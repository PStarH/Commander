...
function makeStubUnified(): UnifiedMemory {
  return {
    ...
    getWorkingMemory: vi
      .fn()
      .mockReturnValue({ add: vi.fn(), getWorkingContext: vi.fn().mockReturnValue([]) }),
...
describe('MemorySystem.assertNamespaced', () => {
  ...
  it("blocks writes to another agent's namespace", () => {
    const ms = new MemorySystem({ unified: makeStubUnified() });
    expect(() => ms.assertNamespaced('agent-1', 'agents/agent-2/episodic/abc')).toThrow(
      /MEMORY-001/,
    );
  });
  ...
  it('allows cross-namespace writes when ACL grants the namespace', () => {
    const ms = new MemorySystem({ unified: makeStubUnified() });
    expect(() =>
      ms.assertNamespaced('agent-1', 'agents/agent-2/episodic/abc', {
        role: 'collaborator',
  ...
  it('allows writes to shared tasks/ namespace when ACL grants tasks', () => {
    const ms = new MemorySystem({ unified: makeStubUnified() });
    expect(() =>
      ms.assertNamespaced('agent-1', 'tasks/T-42/logs', {
        role: 'task-member',
        namespaces: ['tasks'],
      }),
    ).not.toThrow();
  ...
  it('fail-closed: empty ACL namespaces blocks all cross-namespace writes', () => {
    const ms = new MemorySystem({ unified: makeStubUnified() });
    expect(() =>
      ms.assertNamespaced('agent-1', 'agents/agent-2/x', {
        role: 'restricted',
        namespaces: [],
      }),
    ).toThrow(/MEMORY-001/);
