...
describe('isUrlSafe', () => {
  ...
  it('blocks localhost variants', () => {
    for (const host of ['http://localhost', 'http://localhost:3000', 'http://127.0.0.1', 'http://127.0.0.1:8080', 'http://0.0.0.0', 'http://[::1]']) {
  ...
  it('blocks private IPv4 ranges', () => {
    for (const host of ['http://10.0.0.1', 'http://10.255.255.255', 'http://172.16.0.1', 'http://172.31.255.255', 'http://192.168.1.1', 'http://169.254.1.1']) {
  ...
  it('blocks common internal service ports', () => {
    for (const port of [6379, 27017, 5432, 9200, 11211, 8500, 8300, 8501]) {
      assert.strictEqual(isUrlSafe(`http://example.com:${port}`).safe, false, `expected port ${port} to be blocked`);
    }
  });

  it('allows public URLs with default ports', () => {
    for (const url of ['https://example.com', 'https://example.com:443', 'http://example.com:80', 'https://api.github.com/users']) {
...
describe('safeFetch', () => {
  ...
  it('truncates responses exceeding maxBytes', async () => {
    ...
    const port = (server.address() as { port: number }).port;
    try {
      const result = await performFetch(`http://127.0.0.1:${port}/`, { timeoutMs: 5000, maxBytes: 100 });
...
describe('atomicWriteFile', () => {
  ...
  it('produces unique tmp filenames for concurrent invocations', async () => {
    ...
    assert.strictEqual(fs.readFileSync(target, 'utf-8'), 'x', 'final content should be one of the writes');
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
