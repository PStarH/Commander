...
async function main() {
  ...
  const hasBlockedInOutput = output.includes('GUARDIAN_BLOCKED');
  const hasBlockedInResult =
    result.error?.includes('GUARDIAN_BLOCKED') || result.summary.includes('GUARDIAN_BLOCKED');
  if (!hasBlockedInOutput && !hasBlockedInResult) {
