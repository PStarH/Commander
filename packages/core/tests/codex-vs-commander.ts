...
const TASKS: ComparisonTask[] = [
  {
    ...
    category: 'research',
    prompt:
      'Research Raft vs Paxos consensus algorithms. Compare fault tolerance, performance, and complexity. Write a comparison table.',
    codexPrompt:
      'Research Raft vs Paxos consensus algorithms. Compare fault tolerance, performance, and complexity. Write a comparison table to /tmp/compare-consensus.md',
    commanderGoal:
      'Research Raft vs Paxos consensus algorithms. Compare fault tolerance, performance, and complexity. Write a comparison table to /tmp/compare-consensus.md',
  },
  {
    ...
    category: 'analysis',
    prompt:
      'Analyze the packages/core/src/runtime/ directory. List the main modules, their responsibilities, and key design patterns used.',
    codexPrompt:
      'Analyze the packages/core/src/runtime/ directory. List the main modules, their responsibilities, and key design patterns used. Write findings to /tmp/compare-runtime-analysis.md',
    commanderGoal:
      'Analyze the packages/core/src/runtime/ directory. List the main modules, their responsibilities, and key design patterns used. Write findings to /tmp/compare-runtime-analysis.md',
  },
  {
    ...
    category: 'coding',
    prompt:
      'Write a TypeScript function that implements a LRU cache with get/set operations, O(1) time complexity, using a doubly-linked list and Map.',
    codexPrompt:
      'Write a TypeScript function that implements a LRU cache with get/set operations, O(1) time complexity, using a doubly-linked list and Map. Write the implementation to /tmp/compare-lru-cache.ts with proper types and tests.',
    commanderGoal:
      'Write a TypeScript function that implements a LRU cache with get/set operations, O(1) time complexity, using a doubly-linked list and Map. Write the implementation to /tmp/compare-lru-cache.ts with proper types and tests.',
  },
  {
    ...
    category: 'creative',
    prompt:
      'Write a technical blog post about how multi-agent AI systems can use Critical Path Method for task scheduling. Include code examples.',
    codexPrompt:
      'Write a technical blog post about how multi-agent AI systems can use Critical Path Method for task scheduling. Include code examples. Write to /tmp/compare-blog-cpm.md',
    commanderGoal:
      'Write a technical blog post about how multi-agent AI systems can use Critical Path Method for task scheduling. Include code examples. Write to /tmp/compare-blog-cpm.md',
  },
  {
    ...
    category: 'planning',
    prompt:
      'Create a migration plan for moving a TypeScript monorepo to a multi-repo structure. Include dependency analysis, phases, and rollback strategy.',
    codexPrompt:
      'Create a migration plan for moving a TypeScript monorepo to a multi-repo structure. Include dependency analysis, phases, and rollback strategy. Write to /tmp/compare-migration-plan.md',
    commanderGoal:
      'Create a migration plan for moving a TypeScript monorepo to a multi-repo structure. Include dependency analysis, phases, and rollback strategy. Write to /tmp/compare-migration-plan.md',
  },
  {
    ...
    category: 'multi',
    prompt:
      'Research error handling patterns in distributed systems, then identify 3 improvements for the error handling in packages/core/src/runtime/ and write a proposal with code examples.',
    codexPrompt:
      'Research error handling patterns in distributed systems, then identify 3 improvements for the error handling in packages/core/src/runtime/ and write a proposal with code examples. Write to /tmp/compare-error-handling.md',
    commanderGoal:
      'Research error handling patterns in distributed systems, then identify 3 improvements for the error handling in packages/core/src/runtime/ and write a proposal with code examples. Write to /tmp/compare-error-handling.md',
  },
];
...
async function runCommanderTask(task: ComparisonTask): Promise<TaskResult> {
  ...
  try {
    ...
    if (fileMatch) {
      try {
        outputSize = fs.statSync(fileMatch[1]).size;
      } catch {}
    }
...
async function main() {
  ...
  for (const task of TASKS) {
    ...
    console.log(
      `  │  ${cmdResult.success ? '✅' : '❌'} ${cmdResult.durationMs / 1000}s | ${cmdResult.tokensUsed.toLocaleString()} tokens | ${cmdResult.outputSize} bytes`,
    );
    ...
    console.log(
      `  │  ${codexResult.success ? '✅' : '❌'} ${codexResult.durationMs / 1000}s | ${codexResult.tokensUsed.toLocaleString()} tokens | ${codexResult.outputSize} bytes${codexResult.error ? ` | ERR: ${codexResult.error.slice(0, 60)}` : ''}`,
    );
  ...
  console.log('                    ─────────        ─────────');
  console.log(
    `  Success Rate:     ${summary.commander.successRate.padEnd(16)} ${summary.codex.successRate}`,
  );
  console.log(
    `  Total Tokens:     ${String(summary.commander.totalTokens).padEnd(16)} ${summary.codex.totalTokens}`,
  );
  console.log(
    `  Total Time:       ${(summary.commander.totalTimeSec + 's').padEnd(16)} ${summary.codex.totalTimeSec}s`,
  );
  console.log(
    `  Avg Time/Task:    ${(summary.commander.avgTimeSec + 's').padEnd(16)} ${summary.codex.avgTimeSec}s`,
  );
  console.log(
    `  Output Generated: ${(summary.commander.totalOutputBytes + ' bytes').padEnd(16)} ${summary.codex.totalOutputBytes} bytes`,
  );
  console.log('');
  ...
  for (const task of TASKS) {
    ...
    console.log(
      `    Commander: ${cmd?.success ? '✅' : '❌'} ${(cmd?.durationMs ?? 0) / 1000}s | ${(cmd?.tokensUsed ?? 0).toLocaleString()} tokens | ${cmd?.outputSize ?? 0} bytes`,
    );
    console.log(
      `    Codex:     ${codex?.success ? '✅' : '❌'} ${(codex?.durationMs ?? 0) / 1000}s | ${(codex?.tokensUsed ?? 0).toLocaleString()} tokens | ${codex?.outputSize ?? 0} bytes${codex?.error ? ` | ERR: ${codex.error.slice(0, 60)}` : ''}`,
    );
  }
