...
describe('1.1 Tool Calling Edge Cases', () => {
  ...
  it('TC-EC-7: ExecPolicy handles empty command', () => {
    ...
    assert.strictEqual(
      result.decision,
      'prompt',
      'Empty command defaults to prompt (fail-safe for unknown commands)',
    );
  });
  ...
  it('TC-EC-8: ExecPolicy handles very long command (10K chars)', () => {
    ...
    assert.ok(
      result.decision === 'allow' || result.decision === 'prompt',
      'Long command should not crash policy engine',
    );
  });
  ...
  it('TC-EC-9: Approval circuit breaker prevents runaway tool spend', async () => {
    ...
    assert.ok(
      deniedCount > 5,
      `Without callback, most destructive ops should be denied (got ${deniedCount}/10)`,
    );
  });
});
...
describe('1.2 Long-term Memory Edge Cases', () => {
  it('LM-EC-1: 500+ noise messages — compaction preserves signal', () => {
    ...
    let msgs: LLMMessage[] = [
      {
        role: 'system',
        content: 'You are a helpful assistant. The user prefers Python over JavaScript.',
      },
    ];
  ...
  it('LM-EC-2: Critical info survives across multiple compactions', () => {
    ...
    const hasSystemPrompt = msgs.some(
      (m) => m.role === 'system' && m.content.toLowerCase().includes('commander'),
    );
        ...
        tool_calls: [
          {
            id: 'tc1',
            type: 'function',
            function: { name: 'test', arguments: JSON.stringify({ key: 'value'.repeat(100) }) },
          },
        ],
      },
    ];
  ...
  it('LM-EC-4: Context window pressure detection', () => {
    ...
    const msgs: LLMMessage[] = [
      ...
      ...Array(20)
        .fill(null)
        .map((_, i) => ({ role: 'user' as const, content: 'x'.repeat(100) })),
    ];
...
describe('1.3 Multimodal Input Edge Cases', () => {
  ...
  it('MM-EC-2: PDF tool handles non-PDF input gracefully', async () => {
    ...
    assert.ok(
      result.includes('Error') || result.includes('PDF'),
      'Non-PDF should return error: ' + result.slice(0, 100),
    );
  });
  ...
  it('MM-EC-3: Vision tool handles non-existent file path', async () => {
    ...
    assert.ok(
      ...
      'Non-existent file should error: ' + result.slice(0, 100),
    );
  });
...
describe('1.4 Self-Correction Edge Cases', () => {
  ...
  it('SC-EC-4: Error classification handles API overload', () => {
    const err = classifyLLMError(
      Object.assign(new Error('529 Service Overloaded'), { status: 529 }),
    );
  ...
  it('SC-EC-5: Error classification handles unknown error safely', () => {
    ...
    assert.ok(
      !err.retryable || err.errorClass !== 'transient',
      'Unknown error should not auto-retry',
    );
  });
...
describe('1.5 Sandbox Execution Edge Cases', () => {
  ...
  it('SX-EC-2: Workspace-write protects sensitive paths', () => {
    ...
    assert.ok(
      ww.filesystem.protectedPaths.includes('.commander'),
      'Workspace-write protects .commander',
    );
  });
  ...
  it('SX-EC-3: Full-access profile has no restrictions', () => {
    ...
    assert.strictEqual(
      fa.filesystem.protectedPaths.length,
      12,
      'Full-access has 12 protected paths (git, auth, cloud, etc.)',
    );
  });
  ...
  it('SX-EC-5: ExecPolicy allows safe development commands', () => {
    ...
    for (const cmd of safe) {
      ...
      assert.ok(
        ...
        `Should not forbid: ${cmd}, got: ${result.decision}`,
      );
    }
  });
