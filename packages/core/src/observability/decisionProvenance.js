...
exports.buildDecisions = buildDecisions;
exports.decisionsSummary = decisionsSummary;
function buildDecisions(trace) {
    var _a, _b, _c;
    const events = trace.events;
    const bySpan = new Map();
    for (const e of events)
        bySpan.set(e.spanId, e);
    const decisions = [];
    for (const e of events) {
        ...
        decisions.push({
            ...
            toolName: typeof e.data.input === 'string'
                ? e.data.input
                : ((_a = args['toolName']) !== null && _a !== void 0 ? _a : String((_b = e.data.input) !== null && _b !== void 0 ? _b : 'unknown')),
            toolArgs: args,
            decisionReason: extractReason(llmEvent),
            llmSpanId: llmEvent === null || llmEvent === void 0 ? void 0 : llmEvent.spanId,
            llmReasoning: llmEvent ? preview(llmEvent.data.output) : undefined,
            llmModel: (_c = llmEvent === null || llmEvent === void 0 ? void 0 : llmEvent.data.modelInfo) === null || _c === void 0 ? void 0 : _c.model,
            thinkDurationMs,
        });
    }
    return decisions;
}
function findPrecedingLlm(trace, toolEvent) {
    const toolStart = new Date(toolEvent.timestamp).getTime();
    const candidates = trace.events.filter((e) => e.type === 'llm_call' && new Date(e.timestamp).getTime() <= toolStart);
    if (candidates.length === 0)
        return undefined;
    return candidates.reduce((latest, e) => new Date(e.timestamp).getTime() > new Date(latest.timestamp).getTime() ? e : latest);
}
function extractReason(llm) {
    var _a;
    if (!llm)
        return 'no preceding LLM call captured';
    ...
    if (out && typeof out === 'object') {
        ...
        if (typeof o['content'] === 'string')
            return o['content'].slice(0, 300);
        if (Array.isArray(o['tool_calls']) && o['tool_calls'].length > 0) {
            const tc = o['tool_calls'][0];
            return `chose tool: ${String((_a = tc['name']) !== null && _a !== void 0 ? _a : 'unknown')}`;
        }
    }
