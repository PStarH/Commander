...
exports.MultiAgentSynthesizer = void 0;
...
class MultiAgentSynthesizer {
    constructor() {
        this.artifactSystem = (0, artifactSystem_1.getArtifactSystem)();
    }
    async synthesize(strategy, config, taskTree, artifacts) {
        const artifactsUsed = [];
        let synthesis;
        switch (strategy) {
            case 'LEAD_SYNTHESIS':
    ...
    async leadSynthesis(taskTree, artifacts, config) {
        ...
        if (completedNodes.length > 1) {
            const totalResultLen = completedNodes.reduce((sum, n) => { var _a, _b; return sum + ((_b = (_a = n.result) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0); }, 0);
            parts.push(`\n## Summary Statistics\n`);
    ...
    async hierarchicalSynthesis(node, config) {
        var _a;
        if (node.isAtomic || node.subtasks.length === 0) {
            return (_a = node.result) !== null && _a !== void 0 ? _a : `No result for: ${node.goal.slice(0, 100)}`;
        }
        const childResults = [];
    ...
    async voteSynthesis(taskTree, artifacts, config) {
        const completed = this.collectCompleted(taskTree);
        if (completed.length === 0)
            return 'No completed results to synthesize.';
        const results = completed
            .map((n) => {
            var _a, _b;
            return ({
                text: (_a = n.result) !== null && _a !== void 0 ? _a : '',
                goal: n.goal,
                length: ((_b = n.result) !== null && _b !== void 0 ? _b : '').length,
        ...
        if (results.length === 0) {
            return completed
                .map((n) => { var _a; return (_a = n.result) !== null && _a !== void 0 ? _a : ''; })
                .filter(Boolean)
                .join('\n\n---\n\n');
        }
        ...
        if (results.length >= 3) {
            ...
            if (uniqueFingerprints.size === 1) {
                ...
                (0, logging_1.getGlobalLogger)().warn('Synthesizer', 'Echo chamber detected: top 3 results have identical openings', {
    ...
    async ensembleSynthesis(taskTree, artifacts, config) {
        const completed = this.collectCompleted(taskTree);
        if (completed.length === 0)
            return 'No completed results to synthesize.';
        ...
        const scored = completed
            .map((n) => {
            var _a;
            const text = (_a = n.result) !== null && _a !== void 0 ? _a : '';
            ...
            const totalScore = lengthScore * ENSEMBLE_LENGTH_WEIGHT +
        ...
        for (const result of scored) {
            const weightLabel = result.score > 0.7
                ? '★ High confidence'
                : result.score > 0.4
                    ? '◆ Medium confidence'
                    : '○ Lower confidence';
    ...
    async runQualityGatesStrict(gates, synthesis, taskTree) {
        return this.runQualityGates(gates, synthesis, taskTree);
    }
    async runQualityGates(gates, synthesis, taskTree) {
        const results = [];
    ...
    checkConsistency(synthesis, taskTree) {
        let score = 1.0;
        ...
        for (const n of completed) {
            if (n.result && n.result.length > 20)
                hasResults++;
        }
    ...
    checkCompleteness(synthesis, taskTree) {
        const completed = this.collectCompleted(taskTree);
    ...
    checkAccuracy(synthesis) {
        ...
        if (synthesis.includes('[citation needed]'))
            score -= CITATION_PENALTY;
        if (synthesis.includes('[source missing]'))
            score -= CITATION_PENALTY;
        return Math.max(0, score);
    }
    checkSafety(synthesis) {
        let score = 1.0;
    ...
    getDepth(node, root, currentDepth = 0) {
        if (node.id === root.id)
            return currentDepth;
...
exports.MultiAgentSynthesizer = MultiAgentSynthesizer;
