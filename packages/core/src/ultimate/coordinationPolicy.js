...
exports.evaluateCoordinationPolicy = evaluateCoordinationPolicy;
const constants_1 = require("../config/constants");
const TOPOLOGY_TOKEN_MULTIPLIER = {
    SINGLE: 1.0,
    SEQUENTIAL: 1.1,
    PARALLEL: 2.0,
    HIERARCHICAL: 3.0,
...
function evaluateCoordinationPolicy(deliberation, candidateTopology, dag, learnedWeights, tenantId) {
    ...
    const parallelismEfficiency = Math.max(0, Math.min(1, (Math.min(agentCount, usefulParallelism) / agentCount) * (1 - coupling * 0.55)));
    const coordinationTokenEstimate = estimateCoordinationTokens(agentCount, coordinationChannels, candidateTopology);
    const coordinationCostUsd = coordinationTokenEstimate * constants_1.COST_PER_TOKEN;
    const latencyPenaltyMs = estimateLatencyPenaltyMs(agentCount, coordinationChannels, coupling, candidateTopology);
    const pattern = selectCoordinationPattern(deliberation, candidateTopology, agentCount, dag);
    const gain = estimateGain(deliberation, candidateTopology, {
        agentCount,
        ...
        coupling,
    }, usefulParallelism, learnedWeights, tenantId);
    const reasons = [];
    const evidence = [
    ...
    if (agentCount <= 1)
        reasons.push('Only one useful worker estimated.');
    if (usefulParallelism < 1.5)
        reasons.push('Task has little independent parallel work.');
    if (coupling >= 0.75)
    ...
    if (gain.netRoi < roiThreshold) {
        reasons.push(`Estimated coordination ROI ${gain.netRoi.toFixed(2)} is below learned threshold ${roiThreshold.toFixed(2)}.`);
    }
    const lowValueLocalTask = deliberation.taskType === 'FACTUAL' &&
    ...
    const weakNoEvidenceCase = gain.netRoi < roiThreshold &&
        !hasPositiveCoordinationSignal(deliberation, candidateTopology, dag);
    const negativeRoi = candidateTopology !== 'SINGLE' &&
...
function inferCoupling(deliberation, dag) {
    if (dag)
        return dag.metadata.interSubtaskCoupling;
...
function inferUsefulParallelism(deliberation, dag) {
    if (dag)
        return Math.max(1, dag.metadata.parallelismWidth);
...
function estimateCoordinationTokens(agentCount, coordinationChannels, topology) {
    if (topology === 'SINGLE')
        return 0;
...
function estimateLatencyPenaltyMs(agentCount, coordinationChannels, coupling, topology) {
    if (topology === 'SINGLE')
        return 0;
...
function estimateGain(deliberation, topology, overhead, usefulParallelism, learnedWeights, tenantId) {
    ...
    const contextGain = deliberation.estimatedTokens > 50000
        ? 0.2
        : deliberation.estimatedTokens > 20000
            ? 0.12
            : deliberation.estimatedTokens > 8000
                ? 0.06
                : 0;
    ...
    const latencyGain = deliberation.taskNature === 'IO_BOUND' && usefulParallelism > 1
        ? Math.min(0.25, (1 - 1 / Math.max(1, usefulParallelism)) * 0.3)
        : 0;
    const overheadPenalty = Math.min(0.6, (overhead.tokenMultiplier - 1) * 0.08 +
...
function structuralSignal(deliberation, topology) {
    if ((topology === 'HIERARCHICAL' || topology === 'HYBRID') &&
        (deliberation.taskType === 'RESEARCH' || deliberation.taskType === 'REASONING')) {
        return 0.08;
    }
    return 0;
}
function breadthSignal(deliberation, usefulParallelism) {
    if (usefulParallelism <= 1)
        return 0;
    if (deliberation.taskType === 'RESEARCH')
        return 0.12;
    if (deliberation.taskType === 'ANALYSIS')
        return 0.08;
    if (deliberation.taskType === 'CREATIVE')
        return 0.08;
    if (deliberation.taskNature === 'IO_BOUND')
        return 0.08;
    return 0.03;
}
function isolationSignal(deliberation, topology) {
    const needsReview = deliberation.taskType === 'CODING' || deliberation.taskType === 'ANALYSIS';
    if (topology === 'EVALUATOR_OPTIMIZER' && needsReview)
        return 0.08;
    if (topology === 'HANDOFF' && deliberation.capabilitiesNeeded.length > 2)
        return 0.06;
    if (topology === 'DEBATE' && deliberation.taskType === 'REASONING')
        return 0.05;
    if (topology === 'ENSEMBLE' && deliberation.taskType === 'CREATIVE')
        return 0.05;
    return 0;
}
function selectCoordinationPattern(deliberation, topology, agentCount, dag) {
    if (topology === 'SINGLE' || agentCount <= 1)
        return 'SINGLE_AGENT';
    if (topology === 'EVALUATOR_OPTIMIZER')
        return 'REVIEWER';
    if (topology === 'HIERARCHICAL' || topology === 'HYBRID')
    ...
    if (topology === 'CONSENSUS')
        return 'CONSENSUS';
    if (agentCount === 2 || (dag === null || dag === void 0 ? void 0 : dag.metadata.criticalPathDepth) === 2)
        return 'PLANNER_EXECUTOR';
    if (topology === 'PARALLEL')
        return 'SPECIALIST_SWARM';
    return deliberation.taskType === 'CODING' ? 'PLANNER_EXECUTOR' : 'SPECIALIST_SWARM';
}
function selectMode(agentCount, pattern) {
    if (pattern === 'SINGLE_AGENT' || agentCount <= 1)
        return 'single';
    if (agentCount <= 2 || pattern === 'PLANNER_EXECUTOR' || pattern === 'REVIEWER')
        return 'two_agent';
    return 'swarm';
}
function chooseFallbackTopology(deliberation, coupling, _) {
    if (coupling >= 0.7 && deliberation.estimatedAgentCount > 1)
        return 'SEQUENTIAL';
    return 'SINGLE';
}
function hasPositiveCoordinationSignal(deliberation, topology, dag) {
    if (deliberation.taskType === 'RESEARCH' && deliberation.estimatedAgentCount >= 3)
        return true;
    if (deliberation.taskType === 'ANALYSIS' && deliberation.estimatedAgentCount >= 3)
        return true;
    if (deliberation.taskType === 'REASONING' && deliberation.estimatedAgentCount >= 5)
        return true;
    if (deliberation.taskType === 'CREATIVE' && deliberation.estimatedAgentCount >= 3)
        return true;
    if (deliberation.taskNature === 'IO_BOUND' && deliberation.estimatedAgentCount >= 2)
        return true;
    ...
    if (dag && dag.metadata.parallelismWidth > 3 && dag.metadata.interSubtaskCoupling < 0.7)
        return true;
    if (topology === 'EVALUATOR_OPTIMIZER' && deliberation.taskType === 'CODING')
        return true;
    if (topology === 'DEBATE' && deliberation.taskType === 'REASONING')
        return true;
    if (topology === 'ENSEMBLE' && deliberation.taskType === 'CREATIVE')
