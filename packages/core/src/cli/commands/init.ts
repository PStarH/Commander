...
function statusIcon(status: ConnectivityResult['status']): string {
  switch (status) {
    case 'reachable': return `${$.green}●${$.reset}`;
    case 'auth_error': return `${$.yellow}▲${$.reset}`;
    case 'timeout':   return `${$.yellow}⏱${$.reset}`;
    case 'unreachable': return `${$.red}✗${$.reset}`;
...
function statusLabel(status: ConnectivityResult['status']): string {
  switch (status) {
    case 'reachable': return `${$.green}OK${$.reset}`;
    case 'auth_error': return `${$.yellow}AUTH${$.reset}`;
    case 'timeout':   return `${$.yellow}T/O${$.reset}`;
    case 'unreachable': return `${$.red}DOWN${$.reset}`;
    case 'skipped':   return `${$.dim}--${$.reset}`;
  }
}
...
function tierBadge(tier: string): string {
  switch (tier) {
    case 'local':   return `${$.cyan}local${$.reset}`;
    case 'cloud':   return `${$.dim}cloud${$.reset}`;
    case 'premium': return `${$.yellow}premium${$.reset}`;
    default:        return tier;
  }
}
...
export async function cmdInit(flags: Record<string, string> = {}): Promise<void> {
  ...
  // ── Header ──────────────────────────────────────────────────────────
  console.log(`\n  ${$.bold}${$.blue}╭──────────────────────────────────────────────────────────╮${$.reset}`);
  console.log(`  ${$.bold}${$.blue}│${$.reset}  ${$.bold}Commander Init${$.reset} — Zero-Config Environment Setup          ${$.bold}${$.blue}│${$.reset}`);
  ...
  section('ENVIRONMENT');
  kv('API keys found', `${probe.apiProviderCount}`, probe.apiProviderCount > 0 ? $.green : $.yellow);
  kv('Ollama', probe.ollamaAvailable ? `${$.green}available${$.reset}` : `${$.dim}not found${$.reset}`);
  kv('vLLM', probe.vllmAvailable ? `${$.green}available${$.reset}` : `${$.dim}not found${$.reset}`);
  kv('Docker', probe.dockerAvailable ? `${$.green}available${$.reset}` : `${$.dim}not found${$.reset}`);
  kv('Redis', probe.redisUrl ? `${$.green}${probe.redisUrl}${$.reset}` : `${$.dim}not configured${$.reset}`);
  kv('Kubernetes', probe.inKubernetes ? `${$.green}in-cluster (${probe.k8sNamespace ?? 'default'})${$.reset}` : `${$.dim}not detected${$.reset}`);
  console.log();
  ...
  const extraProviders = ['openai', 'anthropic', 'google', 'deepseek', 'glm', 'mimo', 'xiaomi',
    'openrouter', 'cohere', 'mistral', 'groq', 'together', 'perplexity', 'fireworks',
    'replicate', 'bedrock', 'xai', 'anyscale', 'deepinfra', 'agnes'];
  ...
  if (!skipTests && allProviderNames.length > 0) {
    console.log(`  ${$.dim}Testing connectivity to ${allProviderNames.length} providers (${timeout}ms timeout)...${$.reset}\n`);
    ...
    console.log(`  ${$.dim}${elapsed}ms — ${$.green}${reachable} reachable${$.reset}${authErrors > 0 ? `, ${$.yellow}${authErrors} auth errors${$.reset}` : ''}${$.reset}\n`);
  } else if (skipTests) {
  ...
  if (results.length > 0) {
    ...
    if (chain.length === 0) {
      ...
      console.log(`  ${$.dim}Run ${$.cyan}commander quickstart${$.reset}${$.dim} for setup guidance.${$.reset}\n`);
      return;
    }

    // Show chain visualization
    const chainDisplay = chain.map((p, i) => {
    ...
    try {
      ...
      console.log(`  ${$.green}✓${$.reset} Configuration saved to ${$.cyan}.commander.json${$.reset}`);
      console.log(`  ${$.dim}  Provider: ${$.cyan}${primary}${$.reset}${$.dim}  Model: ${$.cyan}${primaryResult.defaultModel}${$.reset}`);
  ...
  if (!saveOnly) {
    ...
    console.log(`    ${$.cyan}commander run \"<task>\"${$.reset}        ${$.dim}Execute a task${$.reset}`);
    console.log(`    ${$.cyan}commander run \"<task>\" --dry-run${$.reset} ${$.dim}Plan without executing${$.reset}`);
    console.log(`    ${$.cyan}commander run \"<task>\" --stream${$.reset}  ${$.dim}Real-time progress${$.reset}`);
    console.log(`    ${$.cyan}commander status${$.reset}              ${$.dim}System status${$.reset}`);
    console.log(`    ${$.cyan}commander config${$.reset}              ${$.dim}Manage configuration${$.reset}`);
    console.log(`    ${$.cyan}commander doctor${$.reset}              ${$.dim}Run diagnostics${$.reset}`);
    console.log(`    ${$.cyan}commander tui${$.reset}                 ${$.dim}Interactive dashboard${$.reset}`);
    console.log();
  }
}
...
function getEnvKeyForProvider(provider: string): string | undefined {
  const map: Record<string, string> = {
    ...
    glm: 'ZHIPU_API_KEY', mimo: 'MIMO_API_KEY', xiaomi: 'XIAOMI_API_KEY',
    openrouter: 'OPENROUTER_API_KEY', cohere: 'CO_API_KEY',
    mistral: 'MISTRAL_API_KEY', groq: 'GROQ_API_KEY',
    together: 'TOGETHER_API_KEY', perplexity: 'PERPLEXITY_API_KEY',
    fireworks: 'FIREWORKS_API_KEY', replicate: 'REPLICATE_API_TOKEN',
    xai: 'XAI_API_KEY', anyscale: 'ANYSCALE_API_KEY',
    deepinfra: 'DEEPINFRA_API_KEY', agnes: 'AGNES_API_KEY',
  };
  return map[provider];
}
