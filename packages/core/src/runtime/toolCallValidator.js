...
exports.formatValidationErrorsJson = formatValidationErrorsJson;
...
function compileSchema(schema) {
    ...
    const propertyEnums = new Map();
    const propertyConstraints = new Map();
    const defaults = new Map();
    for (const [key, propSchema] of Object.entries(properties)) {
        if (propSchema.type) {
            propertyTypes.set(key, propSchema.type);
        }
 ...
 */
function validateToolCall(args, schema) {
    const errors = [];
    const repaired = { ...args };
    const repairs = [];
    ...
    for (const [key, allowedValues] of Array.from(schema.propertyEnums)) {
        ...
        const normalizedAllowed = allowedValues.map((v) => typeof v === 'string' ? v.toLowerCase() : v);
 ...
 */
function tryCoerce(value, expectedType) {
    ...
    if (expectedType === 'array') {
        if (typeof value === 'string') {
            try {
                ...
                (0, logging_1.getGlobalLogger)().debug('ToolCallValidator', 'Failed to parse array coercion JSON', {
 ...
 */
function formatValidationErrors(errors, toolName, repairs) {
 ...
 */
function formatValidationErrorsJson(errors, toolName, repairs, repairedArgs) {
    return {
        tool: toolName,
        valid: errors.length === 0,
        errors: errors.map((e) => {
            ...
            return ({
                ...
                suggestion: (_a = e.suggestion) !== null && _a !== void 0 ? _a : defaultSuggestion(e),
            });
        }),
        repairs: repairs !== null && repairs !== void 0 ? repairs : [],
        repairedArgs,
