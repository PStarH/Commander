...
exports.isConfidentResponse = isConfidentResponse;
exports.hasInformationGain = hasInformationGain;
/**
 ...
 */
function isConfidentResponse(response) {
...
function hasLowEntropySignals(content) {
    ...
    const lowEntropyScore = lowEntropyMarkers.reduce((score, marker) => score + (contentLower.includes(marker) ? 1 : 0), 0);
    const sentences = trimmed.split(/[.!?]+/).filter((s) => s.trim().length > 0);
    const avgSentenceLength = sentences.length > 0 ? trimmed.length / sentences.length : trimmed.length;
 ...
 */
function hasInformationGain(toolCalls, recentResults) {
