...
exports.now = now;
exports.delay = delay;
exports.descendingToolOrder = descendingToolOrder;
exports.applyObservationMask = applyObservationMask;
 ...
 */
function descendingToolOrder(toolCalls) {
...
async function applyObservationMask(toolResults, windowSize) {
    if (windowSize <= 0 || toolResults.length <= windowSize)
        return toolResults;
    ...
    const processed = await Promise.all(toolResults.map(async (r, i) => {
        if (i < toolResults.length - windowSize && !r.error && r.output.length > 100) {
            const purified = (0, observationPurifier_1.purifyObservation)(r.output, r.name);
