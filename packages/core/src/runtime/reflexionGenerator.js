...
exports.ReflexionGenerator = void 0;
...
const HEURISTIC_PATTERNS = [
    {
        match: (ctx) => /timeout|timed\s*out|TOOL_TIMEOUT|ETIMEDOUT|exceeded\s+\d+\s*ms/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(ENOENT|not\s+found|does\s+not\s+exist|no\s+such\s+file|cannot\s+find|missing)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(EACCES|permission\s+denied|forbidden|unauthorized|401|403|insufficient\s+permissions)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(invalid\s+(argument|param|input|format)|validation\s+failed|missing\s+required|required\s+field|schema\s+validation)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(TypeError|cannot\s+read\s+(property|properties)|is\s+not\s+a\s+function|undefined\s+is\s+not|null\s+is\s+not)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(SyntaxError|Unexpected\s+token|JSON\.parse|invalid\s+JSON|unexpected\s+end\s+of\s+JSON|malformed)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(ECONNREFUSED|ENETUNREACH|ECONNRESET|EHOSTUNREACH|network\s+error|fetch\s+failed|getaddrinfo|socket\s+hang\s+up)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(409|conflict|already\s+exists|duplicate\s+key|UNIQUE\s+constraint)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(ENOSPC|out\s+of\s+(memory|disk|space)|heap\s+space|allocation\s+failed|memory\s+exhausted)\b/i.test(ctx.error),
        reflect: (ctx) => ({
    ...
    {
        match: (ctx) => /\b(circuit\s+breaker|circuit\s+is\s+open|service\s+unavailable|503)\b/i.test(ctx.error),
        reflect: (ctx) => ({
...
class ReflexionGenerator {
    constructor(llmProvider, options) {
        ...
        this.options = { ...DEFAULT_OPTIONS, ...(options !== null && options !== void 0 ? options : {}) };
    }
    /**
    ...
    async generate(ctx) {
        ...
        if (!this.options.heuristicOnly && this.llmProvider) {
            ...
            try {
                ...
                (0, logging_1.getGlobalLogger)().debug('ReflexionGenerator', 'LLM reflexion failed, using generic fallback', {
...
Be specific. Avoid generic advice like "try again" or "be more careful".`;
    ...
    async callLLMWithTimeout(prompt, timeoutMs) {
        if (!this.llmProvider) {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            timer = setTimeout(() => reject(new Error(`Reflexion LLM call timed out after ${timeoutMs}ms`)), timeoutMs);
     ...
     */
    static formatForContext(ctx, reflexion) {
        return [
...
exports.ReflexionGenerator = ReflexionGenerator;
function truncate(s, maxLen) {
    if (s.length <= maxLen)
        return s;
    return s.slice(0, maxLen) + '…';
}
function isReflexionShape(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    return (typeof v.whatFailed === 'string' &&
...
function resolveDefaultModel(provider) {
    ...
    while (current && typeof current === 'object') {
        const candidate = current.defaultModel;
        if (typeof candidate === 'string' && candidate.length > 0)
            return candidate;
        const modelField = current.model;
