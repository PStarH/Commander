...
export class SqliteMemoryStore implements MemoryStore {
  ...
  async init(): Promise<void> {
    ...
    if (!BetterSqlite3) {
      getGlobalLogger().warn(
        'SqliteMemoryStore',
        'better-sqlite3 not available — install with: pnpm add better-sqlite3',
      );
  ...
  private createSchema(): void {
    ...
    const ftsCount = this.db
      ...
      .get<{ cnt: number }>();
    const memCount = this.db.prepare('SELECT COUNT(*) as cnt FROM memories').get<{ cnt: number }>();
  ...
  async update(options: MemoryManageOptions): Promise<EpisodicMemoryItem | null> {
    ...
    if (options.updates) {
      ...
      if (sets.length > 0) {
        this.db!.prepare(
          `UPDATE memories SET ${sets.join(', ')} WHERE id = @id AND project_id = @projectId`,
        ).run(params);
      }
    }
  ...
  async read(id: string, projectId: string): Promise<EpisodicMemoryItem | null> {
    ...
    if (!row) return null;
    this.db!.prepare('UPDATE memories SET last_accessed_at = ? WHERE id = ?').run(
  ...
  async search(query: MemorySearchQuery): Promise<MemorySearchResult> {
    ...
    const countRow = this.db!.prepare(`SELECT COUNT(*) as cnt FROM memories WHERE ${where}`).get<{
      cnt: number;
    }>(...params);
    const total = countRow?.cnt ?? 0;
