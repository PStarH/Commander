...
export class SqliteMemoryStore implements MemoryStore {
  ...
  private stmtWrite: any = null;
  private stmtRead: any = null;
  private stmtDelete: any = null;
  private stmtSearch: any = null;
  private stmtCountByKind: any = null;
  private stmtCountByDuration: any = null;
  private stmtStats: any = null;
  private stmtDeleteExpired: any = null;
  private stmtDeleteByMission: any = null;
  ...
  private rowToItem(row: any): EpisodicMemoryItem {
    return {
      id: row.id,
  ...
  async batchWrite(items: MemoryWriteOptions[]): Promise<EpisodicMemoryItem[]> {
    ...
    const txn = this.db.transaction((ops: MemoryWriteOptions[]) => {
      for (const op of ops) {
        results.push(this.write(op) as any);
      }
    });
    txn(items);
    // write() returns EpisodicMemoryItem resolved by await above
    const awaited: EpisodicMemoryItem[] = [];
    for (const op of items) {
      awaited.push(await this.write(op));
    }
    return awaited;
  }
  ...
  async update(options: MemoryManageOptions): Promise<EpisodicMemoryItem | null> {
    ...
    if (options.updates) {
      const sets: string[] = [];
      const params: any = { id: options.id, projectId: options.projectId };
  ...
  async search(query: MemorySearchQuery): Promise<MemorySearchResult> {
    ...
    return {
      items: rows.map((r: any) => this.rowToItem(r)),
      total,
      query,
    };
  }
  ...
  async getStats(projectId: string): Promise<MemoryStats> {
    ...
    for (const r of rows) byKind[r.kind as MemoryKind] = r.cnt;
    ...
    for (const r of durRows) byDuration[r.duration as MemoryDuration] = r.cnt;
    ...
    const totalItems = rows.reduce((s: number, r: any) => s + r.cnt, 0);
