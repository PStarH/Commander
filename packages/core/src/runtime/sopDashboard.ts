...
/**
 * Async variant of listSOPs. Reads SOP manifest files in parallel and skips
 * any per-file ENOENT so a partial agent directory is still surfaced.
 * SSE handlers and the /api/v1/sops* dashboard endpoint should use this
 * instead of the sync version under load.
 */
export async function listSOPsAsync(customDir?: string): Promise<SOPListItem[]> {
  const baseDir = resolveSOPDir(customDir);
  let agentEntries: import('node:fs').Dirent[];
  try {
    agentEntries = await fs.promises.readdir(baseDir, { withFileTypes: true });
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') return [];
    getGlobalLogger().warn('SopDashboard', 'Failed to read SOP base directory', {
      baseDir,
      error: (err as Error)?.message,
    });
    reportSilentFailure(err, 'sopDashboard:listAsync:baseDir');
    return [];
  }
  const items: SOPListItem[] = [];
  const agentDirs = agentEntries.filter((d) => d.isDirectory());
  await Promise.all(
    agentDirs.map(async (agentDir) => {
      ...
      let files: string[];
      try {
        files = await fs.promises.readdir(agentPath);
      } catch (err) {
        getGlobalLogger().warn('SopDashboard', 'Failed to read SOP agent directory', {
          agentPath,
          error: (err as Error)?.message,
        });
        reportSilentFailure(err, 'sopDashboard:listAsync:agentDir');
        return;
      }
      const jsonFiles = files.filter((f) => f.endsWith('.json'));
      await Promise.all(
        jsonFiles.map(async (jsonFile) => {
          ...
          try {
            ...
            getGlobalLogger().warn('SopDashboard', 'Failed to read SOP manifest', {
              jsonPath,
              error: (err as Error)?.message,
            });
            reportSilentFailure(err, 'sopDashboard:listAsync:jsonFile');
            // Corrupt JSON file — skip
          }
        }),
      );
...
/** Async variant of getSOP. */
export async function getSOPAsync(
  agentId: string,
  runId: string,
  customDir?: string,
  ...
  try {
    ...
    reportSilentFailure(err, 'sopDashboard:getSOPAsync');
    return null;
  }
}
...
/** Async variant of getSOPMarkdown. */
export async function getSOPMarkdownAsync(
  agentId: string,
  runId: string,
  customDir?: string,
): Promise<string | null> {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') return null;
    getGlobalLogger().warn('SopDashboard', 'Failed to read SOP markdown', {
      mdPath,
      error: (err as Error)?.message,
    });
    reportSilentFailure(err, 'sopDashboard:getSOPMarkdownAsync');
    return null;
  }
}
...
/** Async variant of getSOPDashboardData. Preferred for SSE stream consumers. */
export async function getSOPDashboardDataAsync(customDir?: string): Promise<SOPDashboardData> {
  const bus = getMessageBus();
  const sops = await listSOPsAsync(customDir);
  const history =
    (bus as { getHistory?: (topic?: string, limit?: number) => BusMessage[] }).getHistory?.(
      'sop.generated',
      50,
...
/**
 * Async variant of renderSOPDashboardHtml. Replaces the synchronous
 * `getSOPDashboardData()` call inside the render function with
 * `getSOPDashboardDataAsync()` so the SOP `/dashboard/sop` HTTP route and
 * SSE bus subscribers do not block the event loop while the renderer reads
 * SOP manifests from disk.
 *
 * Identical output to renderSOPDashboardHtml() — only the data source
 * differs. Caller must `await` the result.
 */
export async function renderSOPDashboardHtmlAsync(customDir?: string): Promise<string> {
  const data = await getSOPDashboardDataAsync(customDir);

  // Mirror the synchronous renderer exactly so the only observable change
  // is async I/O. Logic is verbatim from renderSOPDashboardHtml() with the
  // internal `getSOPDashboardData(customDir)` call replaced.
