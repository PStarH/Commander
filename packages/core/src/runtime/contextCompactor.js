...
exports.ContextCompactor = exports.FailureCorrelationTracker = void 0;
const tokenGovernor_1 = require("./tokenGovernor");
const logging_1 = require("../logging");
const rebuildPrompt_1 = require("./rebuildPrompt");
class FailureCorrelationTracker {
    constructor() {
        this.records = new Map();
        this.globalFingerprints = new Set();
    ...
    record(runId, messages, failureSignal) {
        var _a;
        ...
        for (const msg of messages) {
            ...
            if (msg.tool_calls) {
                for (const tc of msg.tool_calls) {
                    const args = (_a = tc.function.arguments) !== null && _a !== void 0 ? _a : '';
                    if (args.length > 10)
                        fingerprints.add(this.fingerprint(args));
                }
            }
        }
        ...
        if (this.records.size > this.maxRecords) {
            ...
            for (const r of this.records.values()) {
                if (!oldest || r.timestamp < oldest.timestamp)
                    oldest = r;
            }
            if (oldest)
...
exports.FailureCorrelationTracker = FailureCorrelationTracker;
...
function containsInjectionPattern(content) {
    return RE_INJECTION_PATTERNS.some((p) => p.test(content));
}
function redactUnsafeToolContent(content) {
...
function markCompacted(content) {
 ...
 */
function estimateCostWeightedTokens(messages) {
 ...
 */
function estimateMessageCostImpact(msg) {
...
function scoreMessageImportance(msg, index, total, importanceConfig = DEFAULT_PROFILE.importanceConfig, failureTracker) {
    ...
    if (msg.role === 'user') {
        ...
        if (content.length > 20)
            score += importanceConfig.userInstructionBonus;
        // Questions and instructions
        if (RE_QUESTION_INSTRUCTION.test(content))
            score += 0.1;
    }
    ...
    if (msg.role === 'assistant') {
        ...
        if (contentLength > 500)
            score += 0.15; // Long responses are costly to lose
        if (contentLength > 1000)
            score += 0.1; // Extra bonus for very long responses
    }
    ...
    if (isCompacted(msg))
        score += importanceConfig.compactedPenalty;
    ...
    if (failureTracker === null || failureTracker === void 0 ? void 0 : failureTracker.isCorrelated(msg)) {
        score = Math.max(0, score - 0.35);
    }
    return Math.max(0, Math.min(1, score));
}
function analyzeComposition(messages) {
    let toolMessages = 0;
...
function adjustProfileByComposition(profile, composition) {
    const adjusted = {
...
class ContextCompactor {
    constructor(config, failureTracker) {
        ...
        this.failureTracker = failureTracker !== null && failureTracker !== void 0 ? failureTracker : new FailureCorrelationTracker();
    }
    /**
     ...
     */
    recordFailureCorrelation(runId, messages, failureSignal) {
    ...
    needsCompaction(messages, taskType) {
        ...
        if (pct >= layer5Threshold && this.lastWasEmergency)
            return 5;
        if (pct >= adjusted.layer4)
            return 4;
        if (pct >= adjusted.layer3)
            return 3;
        if (pct >= adjusted.layer2)
            return 2;
        if (pct >= adjusted.layer1)
            return 1;
        return null;
    }
    /**
    ...
    compact(messages, provider, taskType) {
        ...
        if (!layer) {
            return {
                ...
                action: {
                    ...
                    taskTypeApplied: taskType !== null && taskType !== void 0 ? taskType : null,
                },
            };
        }
        ...
        switch (layer) {
            ...
            case 5: {
                ...
                (0, logging_1.getGlobalLogger)().info('ContextCompactor', 'Layer 5 rebuild recommended', {
     ...
     */
    async rebuild(runId, goal, phase, stepNumber, systemPrompt, recentUserMessages, tokenUsage) {
        const rebuildPrompt = (0, rebuildPrompt_1.getRebuildPrompt)();
        const params = {
            runId,
            goal,
            phase,
     ...
     */
    async compactAsync(messages, provider, taskType) {
     ...
     */
    async compactWithWorkerOffload(messages, workerPool, provider, taskType) {
        if (provider && this.config.maxContextTokens === DEFAULT_CONFIG.maxContextTokens) {
            const providerLimit = this.getProviderContextLimit(provider);
            if (providerLimit !== this.config.maxContextTokens) {
                this.config = { ...this.config, maxContextTokens: providerLimit };
    ...
    async layerWithWorkerOffload(messages, workerPool, provider, profile, layer) {
        const system = [];
        const turns = [];
        let current = [];
        for (const msg of messages) {
    ...
    async extractImportantMessagesWorker(turns, profile, workerPool) {
        var _a, _b;
        const allMsgs = turns.flat();
        const fingerprints = this.failureTracker
            ? [...((_b = (_a = this.failureTracker.getRunRecord('current')) === null || _a === void 0 ? void 0 : _a.messageFingerprints) !== null && _b !== void 0 ? _b : [])]
            : [];
        const workerResult = await workerPool.execute('compact_score_messages', {
    ...
    async buildSummaryWorker(turns, provider, profile, workerPool) {
        ...
        return workerPool.execute('compact_build_summary', {
     ...
     * Public for testing.
     */
    getEffectiveProfile(taskType, messages) {
    ...
    /** Public for testing */
    getCurrentTaskTypeProfile(taskType) {
        var _a;
        return { ...((_a = ADAPTIVE_PROFILES[taskType]) !== null && _a !== void 0 ? _a : DEFAULT_PROFILE) };
    }
    /**
     * Analyze message composition. Public for testing.
     */
    analyzeComposition(messages) {
    ...
    getProviderContextLimit(provider) {
        ...
        const model = (_c = (_b = (_a = p.modelId) === null || _a === void 0 ? void 0 : _a.toLowerCase) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : '';
        if (model.includes('opus') || model.includes('sonnet') || model.includes('claude'))
            return 200000;
        if (model.includes('gpt-5') || model.includes('gpt-4'))
            return 128000;
    ...
    layer2Microcompact(messages, profile) {
        const before = estimateMessagesTokens(messages);
        let trimmedCount = 0;
        let redactedCount = 0;
    ...
    async layer3CollapseAsync(messages, provider, profile) {
        const system = [];
        const turns = [];
        let current = [];
        for (const msg of messages) {
    ...
    layer3Collapse(messages, provider, profile) {
        const system = [];
        const turns = [];
        let current = [];
        for (const msg of messages) {
    ...
    layer4Autocompact(messages, provider, profile) {
        ...
        const scored = nonSystem.map((msg, i) => {
            ...
            return ({
                msg,
                costImpact: ((_a = this.failureTracker) === null || _a === void 0 ? void 0 : _a.isCorrelated(msg))
        ...
        for (const s of scored) {
            if (usedCostWeightedTokens + s.costImpact > retentionBudget)
                break;
     ...
     */
    extractImportantMessages(turns, profile) {
        const allMsgs = turns.flat();
        const scored = allMsgs.map((msg, i) => ({
            msg,
            index: i,
    ...
    selectTopKByCost(scored, budget) {
        const minHeap = [];
        let totalCost = 0;
    ...
    bubbleUp(heap, i) {
        while (i > 0) {
            const parent = (i - 1) >> 1;
            if (heap[parent].costImpact <= heap[i].costImpact)
                break;
    ...
    bubbleDown(heap, i) {
        ...
        while (true) {
            ...
            const left = 2 * i + 1;
            const right = 2 * i + 2;
            if (left < n && heap[left].costImpact < heap[smallest].costImpact)
                smallest = left;
            if (right < n && heap[right].costImpact < heap[smallest].costImpact)
                smallest = right;
            if (smallest === i)
                break;
    ...
    adjustThresholds(taskType, messages) {
        const profile = this.getEffectiveProfile(taskType, messages);
        let base = {
        ...
        try {
            const governor = (0, tokenGovernor_1.getTokenGovernor)();
            const phase = governor.getState().phase;
            // Under pressure, lower thresholds to compact earlier
            const shift = phase === 'critical' ? 0.15 : phase === 'tight' ? 0.1 : phase === 'moderate' ? 0.05 : 0;
            ...
            (0, logging_1.getGlobalLogger)().warn('ContextCompactor', 'Failed to adjust thresholds', {
                error: e === null || e === void 0 ? void 0 : e.message,
            });
            return base;
        }
    }
    buildStructuredSummary(turns, _provider, profile) {
        var _a;
        const verbosity = (_a = profile === null || profile === void 0 ? void 0 : profile.collapseVerbosity) !== null && _a !== void 0 ? _a : 'balanced';
                ...
                if (msg.role === 'assistant' && msg.tool_calls) {
                    for (const tc of msg.tool_calls) {
                        ...
                        try {
                            ...
                            (0, logging_1.getGlobalLogger)().debug('ContextCompactor', 'Failed to parse tool call arguments', {
                                error: e === null || e === void 0 ? void 0 : e.message,
                            });
                        }
                    }
                ...
                if (msg.role === 'tool') {
                    ...
                    if (c.startsWith('error:') || c.startsWith('tool_error') || c.startsWith('ERROR')) {
                        ...
                        if (!foundFinding && keyFindings.length < maxFindings) {
                            ...
                            if (first)
                                keyFindings.push(first.trim().slice(0, 100));
                        }
                    }
                }
                if (msg.role === 'assistant' && !msg.tool_calls) {
        ...
        if (userGoals)
            parts.push(`Goal: ${userGoals}`);
        if (toolCalls.size > 0)
            parts.push(`Tools: ${[...toolCalls].join(', ')}`);
        if (files.length > 0)
            parts.push(`Files: ${[...new Set(files)].slice(0, maxFiles).join(', ')}`);
    ...
    truncateSmall(content, maxChars) {
    ...
    truncateLarge(content, maxChars) {
    ...
    countNewlinesFast(content, start, end) {
        let count = 0;
     ...
     */
    async llmSummarize(turns, provider, maxSummaryTokens = 500) {
