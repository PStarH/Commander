...
export class ReplicateProvider implements LLMProvider {
  ...
  constructor(config: {
    ...
    this.apiKey = config.apiKey || process.env.REPLICATE_API_TOKEN || process.env.REPLICATE_API_KEY || '';
    this.baseUrl = config.baseUrl ?? process.env.REPLICATE_BASE_URL ?? 'https://api.replicate.com/v1';
    this.defaultModel = config.defaultModel ?? process.env.REPLICATE_MODEL ?? 'meta/meta-llama-3.3-70b-instruct';
  }
