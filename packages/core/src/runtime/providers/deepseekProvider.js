...
class DeepSeekProvider {
    constructor(config) {
        ...
        this.baseUrl = (_a = config.baseUrl) !== null && _a !== void 0 ? _a : 'https://api.deepseek.com';
        this.defaultModel = (_b = config.defaultModel) !== null && _b !== void 0 ? _b : 'deepseek-v4-flash';
    }
    async call(request) {
        var _a, _b;
    ...
    buildBody(request, model) {
        ...
        if (request.tools && request.tools.length > 0) {
            body.tools = formatBridge_1.FormatBridge.adaptToolsForProvider(request.tools, 'deepseek');
    ...
    async handleStreamingResponse(response, model) {
        ...
        if (!reader)
            throw new Error('DeepSeek: No response body from streaming endpoint');
        let content = '';
        ...
        while (true) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    (0, logging_1.getGlobalLogger)().debug('DeepSeekProvider', 'Skipping malformed stream chunk', {
    ...
    parseResponse(data, model) {
        ...
        const toolCalls = (_j = message.tool_calls) === null || _j === void 0 ? void 0 : _j.map((tc) => ({
            id: tc.id,
            name: tc.function.name,
            arguments: JSON.parse(tc.function.arguments || '{}'),
        }));
        return {
            content: (_k = message.content) !== null && _k !== void 0 ? _k : '',
            model,
            usage: tokenUsage,
