...
exports.resolveOllamaBaseUrl = resolveOllamaBaseUrl;
...
function resolveOllamaServerUrl() {
    const host = process.env.OLLAMA_HOST;
    if (host) {
        const prefix = host.startsWith('http://') || host.startsWith('https://') ? '' : 'http://';
...
class OllamaProvider extends baseOpenAICompatible_1.BaseOpenAICompatibleProvider {
    constructor(config) {
        var _a;
        super({
            apiKey: (_a = config.apiKey) !== null && _a !== void 0 ? _a : 'ollama',
    ...
    async call(request) {
        ...
        if (!OllamaProvider.healthCache ||
            ...
            try {
                ...
                (0, logging_1.getGlobalLogger)().warn('OllamaProvider', 'Ollama does not appear to be running. Start it with: ollama serve');
            }
        }
...
OllamaProvider.healthCache = null;
OllamaProvider.HEALTH_TTL_MS = 30000;
