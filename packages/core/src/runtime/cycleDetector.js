...
class CycleDetector {
    constructor(config) {
        ...
        this.driftTracker = new Map();
        this.outputHistory = [];
        this.maxOutputHistory = 5;
     ...
     * 检查新工具调用是否构成循环
     */
    check(toolName, args, stepNumber) {
        const record = { name: toolName, args, stepNumber };
    ...
    checkOutput(output) {
        ...
        if (this.outputHistory.length < 2)
            return { detected: false };
    ...
    static extractFingerprint(text) {
        const words = text
            .toLowerCase()
            .replace(/[^a-z0-9\s]/g, '')
            .split(/\s+/)
    ...
    detectConsecutive(toolName, args, stepNumber) {
        ...
        if (diffParam && (args === null || args === void 0 ? void 0 : args[diffParam]) !== undefined) {
    ...
    detectAlternating() {
        ...
        for (let i = 0; i < window; i++) {
            ...
            const diffVal = diffParam && ((_a = rec.args) === null || _a === void 0 ? void 0 : _a[diffParam]) != null ? String(rec.args[diffParam]) : null;
    ...
    detectDrift(toolName, args) {
        ...
        const count = ((_a = this.driftTracker.get(key)) !== null && _a !== void 0 ? _a : 0) + 1;
        ...
        if (this.driftTracker.size > this.config.maxDriftEntries) {
            ...
            while (this.driftTracker.size > this.config.maxDriftEntries) {
                ...
                if (firstKey !== undefined)
                    this.driftTracker.delete(firstKey);
                else
                    break;
            }
        }
