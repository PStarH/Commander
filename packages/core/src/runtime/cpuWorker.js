...
function scoreMessageImportance(msg, index, total, importanceConfig) {
    let score = 0.5;
    if (msg.role === 'system')
        return 1.0;
    if (msg.role === 'user') {
...
function buildHeuristicSummary(turns, verbosity) {
    ...
    try {
        ...
        switch (type) {
            case 'compact_score_messages': {
                const { messages, importanceConfig } = input;
                result = messages
                    .map((m, index) => ({
                    index,
