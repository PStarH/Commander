...
const SCORED_PATTERNS: TypedPattern[] = [
  ...
  { type: 'code', weight: 2, pattern: /```[\s\S]*?```/ },
  {
    ...
    weight: 2,
    pattern:
      /\b(run|execute|compile|debug|fix|refactor)\b.*\b(code|script|function|module|bug|error)\b/i,
  },
  {
    type: 'code',
    weight: 1,
  ...
  },
  {
    ...
    weight: 3,
    pattern:
      /\b(search|look\s+up|find|retrieve|fetch|browse|scrape)\b.*\b(web|url|http|site|website|page|article)\b/i,
  },
  ...
  },
  {
    ...
    weight: 2,
    pattern:
      /\b(population|capital|located|founded|invented|discovered|president|prime minister)\b/i,
  },
  {
    type: 'search',
    weight: 1,
...
const RE_CALC_BASIC =
  /\b(calculate|compute|sum|total|average|percentage?|multiply|divide|subtract|add)\b/i;
...
const RE_SEARCH_FACTS =
  /\b(population|capital|located|founded|invented|discovered|latest|news|current)\b/i;
const RE_SEARCH_OPINION = /\b(you|your|meaning|opinion|think|believe|feel)\b/i;

const RE_FILE_ACTION =
  /\b(read|open|load|parse|analyze|examine)\b.*\b(file|data|csv|json|xml|txt|log|config)\b/i;
const RE_FILE_CONTENTS =
  /\b(contents? of|list|show me|display)\b.*\b(file|directory|folder|path)\b/i;
...
const RE_CODESEARCH_ACTION =
  /\b(find|search|locate|count|list)\b.*\b(comment|code|pattern|symbol|function|class|import)\b/i;
...
export function classifyProvisionIntent(goal: string): {
  bestIntent: keyof ProvisionIntentScores | null;
  scores: ProvisionIntentScores;
} {
  const lower = goal.toLowerCase();
  const scores: ProvisionIntentScores = {
    calculation: 0,
    web_search: 0,
    file_read: 0,
    code_exec: 0,
  ...
  if (
    RE_QUESTION_END.test(goal.trim()) &&
    !scores.calculation &&
    !lower.includes('code') &&
