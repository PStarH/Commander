...
exports.listSOPs = listSOPs;
exports.getSOP = getSOP;
exports.getSOPMarkdown = getSOPMarkdown;
exports.getSOPDashboardData = getSOPDashboardData;
exports.renderSOPDashboardHtml = renderSOPDashboardHtml;
/**
 ...
 */
function getSOP(agentId, runId, customDir) {
    const baseDir = resolveSOPDir(customDir);
 ...
 */
function getSOPMarkdown(agentId, runId, customDir) {
    const baseDir = resolveSOPDir(customDir);
    const safeAgent = sanitizeSegment(agentId);
    const safeRun = sanitizeSegment(runId);
 ...
 */
function getSOPDashboardData(customDir) {
    var _a, _b;
    const bus = (0, messageBus_1.getMessageBus)();
    const sops = listSOPs(customDir);
    // Collect recent bus events related to SOPs
    const history = ((_b = (_a = bus).getHistory) === null || _b === void 0 ? void 0 : _b.call(_a, 'sop.generated', 50)) || [];
 ...
 */
function renderSOPDashboardHtml(customDir) {
    ...
    for (const sop of data.sops) {
        if (sop.stepCount < 10)
            stepBuckets['<10']++;
        else if (sop.stepCount <= 25)
            stepBuckets['10-25']++;
        else if (sop.stepCount <= 50)
            stepBuckets['26-50']++;
        else if (sop.stepCount <= 100)
            stepBuckets['51-100']++;
        else
            stepBuckets['100+']++;
    }
    ...
    const tableRows = data.sops
        .map((sop, i) => `<tr class="sop-row" data-index="${i}" onclick="toggleDetail(${i})">
        ...
        .join(' ')}${sop.tags.length > 3 ? ` <span class="tag-more">+${sop.tags.length - 3}</span>` : ''}</td>
