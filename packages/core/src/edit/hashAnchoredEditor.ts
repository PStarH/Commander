...
import { getTokenMetrics } from './tokenMetrics';
...
export function applyHashEdit(section: HashEditSection): HashEditApplyResult {
  ...
  for (const op of section.ops) {
    // Estimate old content length for the replaced lines
    const oldContentLength = content
      .split('\n')
      .slice(
        op.hashes.length > 0
          ? (anchors.find((a) => a.hash === op.hashes[0])?.lineNumber ?? 1) - 1
          : 0,
      )
      .join('\n').length;
    metrics.recordEdit({
      hashesCount: op.hashes.length,
      replacementLength: op.replacement.length,
      oldContentLength: Math.min(oldContentLength, content.length),
      success: true,
      isRetry: false,
    });
  }
