...
exports.parseHashline = parseHashline;
exports.applyHashlineSection = applyHashlineSection;
exports.formatHashlineHeader = formatHashlineHeader;
exports.formatNumberedLines = formatNumberedLines;
exports.isHashlineFormat = isHashlineFormat;
...
function parseOpHeader(line, lineNum) {
    ...
    if (replaceMatch) {
        ...
        if (start < 1)
            return { error: `Line ${lineNum}: Line numbers are 1-indexed, got ${start}` };
        if (end < start)
 ...
 * 4. Write the result
 */
function applyHashlineSection(section) {
    ...
    // 2. Validate hash
    const currentHash = (0, snapshotStore_1.computeFileHash)(content);
 ...
 */
function applyOperations(lines, ops) {
    var _a, _b;
    const warnings = [];
    let replacements = 0;
    ...
    for (const op of sorted) {
        switch (op.type) {
            case 'replace': {
                ...
                const end = ((_a = op.endLine) !== null && _a !== void 0 ? _a : op.startLine) - 1;
            ...
            case 'delete': {
                ...
                const end = ((_b = op.endLine) !== null && _b !== void 0 ? _b : op.startLine) - 1;
                if (start < 0 || start >= result.length) {
                    return {
                        lines: result,
                        error: `Line ${op.startLine} does not exist (file has ${result.length} lines)`,
 ...
 */
function applyWithRecovery(section, snapshot, currentContent) {
    const warnings = [
    ...
    // Update snapshot
    (0, snapshotStore_1.getSnapshotStore)().record(section.filePath, newContent);
    return {
        success: true,
        filePath: section.filePath,
        newContent,
 ...
 */
function formatHashlineHeader(filePath, hash) {
    return `${FILE_PREFIX}${filePath}${HASH_SEP}${hash}`;
}
/**
 * Format numbered lines in hashline display format.
 */
function formatNumberedLines(content, startLine = 1) {
    const lines = content.split('\n');
 ...
 */
function isHashlineFormat(input) {
