  ...
  for (const exp of exps) {
    try {
      ...
      if (!validateShape<{
        ...
        getGlobalLogger().warn('TrajectoryAnalyzer', 'LLM response failed shape validation, skipping');
        continue;
      }
      const parsed = raw;
      ...
      getGlobalLogger().warn('TrajectoryAnalyzer', 'LLM classification failed, falling back to heuristic', { error: (err as Error)?.message });
      results.push({
        runId: exp.runId ?? exp.id,
        taskType: exp.taskType,
        modelUsed: exp.modelUsed,
