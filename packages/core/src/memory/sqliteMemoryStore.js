...
export class SqliteMemoryStore {
    ...
    `);
        ...
        this.stmtDeleteByMission = d.prepare('DELETE FROM memory_items WHERE mission_id = ? AND project_id = ?');
        this.stmtDeleteExpired = d.prepare('DELETE FROM memory_items WHERE project_id = ? AND expires_at IS NOT NULL AND expires_at < ?');
    ...
    async write(options) {
        ...
        if (options.agentId)
            priority += 5;
        if (options.evidenceRefs?.length)
    ...
    async batchWrite(items) {
        await this.init();
        const results = [];
        const txFn = this.ensureInitialized().transaction;
        const insertMany = txFn((batch) => {
    ...
    async read(id, projectId) {
        await this.init();
        const row = this.stmtGet.get(id, projectId);
        if (!row)
            return null;
    ...
    async update(options) {
        ...
        const row = this.stmtGet.get(options.id, options.projectId);
    ...
    async search(query) {
        ...
        if (query.query && query.query.trim()) {
            ...
            items = items.filter((item) => item.title.toLowerCase().includes(lowerQuery) ||
    ...
    async searchSemantic(query, projectId, limit = 10) {
        ...
        try {
            ...
            const rows = this.stmtFtsSearch.all(ftsQuery, projectId, new Date().toISOString(), limit);
        ...
        LIMIT ?
      `)
                .all(projectId, `%${lowerQuery}%`, `%${lowerQuery}%`, limit);
            return rows.map((r) => this.rowToItem(r));
        }
    }
    // ============================================================================
    ...
    async getStats(projectId) {
        await this.init();
        const row = this.stmtGetStats.get(projectId);
        if (!row) {
            return {
                totalItems: 0,
    ...
    `)
        ...
        for (const tagRow of tagRows) {
            try {
                const tags = JSON.parse(tagRow.tags);
    ...
    rowToItem(row) {
        ...
        try {
            evidenceRefs = row.evidence_refs ? JSON.parse(row.evidence_refs) : undefined;
        }
        catch (err) {
