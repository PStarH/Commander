...
exports.MemoryQualityGate = void 0;
exports.quickQualityCheck = quickQualityCheck;
const embedding_js_1 = require("../runtime/embedding.js");
...
class MemoryQualityGate {
     ...
     */
    async evaluate(entry, embedStore, queryEmbedding) {
     ...
     */
    async checkDuplicate(content, embedStore, queryEmbedding) {
        ...
        for (const entry of entries) {
            ...
            if (!entryEmbedding)
                continue;
            const similarity = (0, embedding_js_1.cosineSimilarity)(queryEmbedding, entryEmbedding);
...
exports.MemoryQualityGate = MemoryQualityGate;
/**
 * Lightweight quality check for fast path (no async, no embedding)
 *
 * Token cost: 0
 */
function quickQualityCheck(content, importance) {
    // Too short
    if (content.length < 2)
        return false;
    // Too low importance
    ...
    if (/^<tool_call>.*<\/tool>$/s.test(content.trim()))
        return false;
    // Error stack
    if (content.includes('at Object.<anonymous>'))
        return false;
    return true;
}
