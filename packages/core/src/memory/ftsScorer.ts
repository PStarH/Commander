 ...
 */
export function tokenizeForBM25(text: string, minLength: number = 2): string[] {
...
export class BM25Scorer {
  ...
  addDocument(id: string, text: string, fieldTexts?: Map<string, string>): void {
    ...
    const doc: BM25Document = { id, tokens, fieldTokens };
   ...
   */
  score(query: string, limit: number = 10): BM25Result[] {
   ...
   */
  private scoreDocument(doc: BM25Document, queryTerms: string[]): number {
   ...
   */
  getStats(): { documents: number; terms: number; avgDocLength: number } {
    return {
      documents: this.N,
      terms: this.df.size,
  ...
  static deserialize(data: {
    ...
    documents: Array<{ id: string; tokens: string[]; fieldTokens?: Array<[string, string[]]> }>;
    N: number;
    totalTokens: number;
    avgDocLength: number;
  }): BM25Scorer {
