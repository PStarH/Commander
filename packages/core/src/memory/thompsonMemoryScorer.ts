...
const DEFAULT_CONFIG: ThompsonScorerConfig = {
  priorAlpha: 1,
  priorBeta: 1,
...
export class ThompsonMemoryScorer {
  ...
  private config: ThompsonScorerConfig;
  ...
  sampleUsefulness(memoryId: string): number {
    ...
    if (!entry) return 0.5; // Default for untracked memories
  ...
  calculateSurprise(memoryId: string, actualOutcome: boolean): number {
    const entry = this.usefulnessMap.get(memoryId);
    if (!entry) return 0.5; // Maximum surprise for unknown memories
  ...
  getEvictionCandidates(): string[] {
    ...
    for (const [id, entry] of this.usefulnessMap) {
      ...
      if (
        mean < this.config.evictionThreshold &&
