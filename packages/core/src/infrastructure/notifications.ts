...
export class NotificationManager {
  ...
  // ── Runtime alert subscription ──────────────────────────────────────────

  private alertSubscriptionActive = false;

  /**
   * Start listening to `system.alert` events on the MessageBus and
   * automatically send notifications when alerts are published.
   *
   * This closes the gap where SLO violations, anomaly detections, and
   * health check degradations were published to the bus but never
   * triggered Slack/Discord notifications — the NotificationManager
   * was only reachable via manual CLI `commander notify` commands.
   *
   * Should be called once at startup from serviceInitializer.
   */
  startAlertListener(): void {
    if (this.alertSubscriptionActive) return;
...
// (Singleton already defined below — using existing getNotificationManager)
