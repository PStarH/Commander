...
exports.ExecuteScriptTool = void 0;
...
class ExecuteScriptTool {
    constructor() {
            ...
            examples: [
                {
                    name: 'execute_script',
                    arguments: {
                        script: 'const files = await tools.file_search({ pattern: "src/**/*.ts" }); console.log("Found", files.length, "files");',
                    },
                },
                {
                    name: 'execute_script',
                    arguments: {
                        script: 'const r = await tools.web_search({ query: "AI news" }); await tools.file_write({ path: "result.txt", content: r }); console.log("Saved");',
        ...
        this.isConcurrencySafe = false; // scripts can mutate shared state
        this.isReadOnly = false; // scripts can call write tools like file_write, git, apply_patch
    ...
    async execute(args) {
        var _a, _b;
        const script = String((_a = args.script) !== null && _a !== void 0 ? _a : '');
    ...
    async runScript(script, tools, console_) {
         ...
         */
        function shadowBlockedProperties(fn) {
        ...
        const safeProxyHandler = {
            get(obj, prop) {
                ...
                if (typeof val === 'function') {
                    const wrapper = (...args) => {
                        ...
                        if (result &&
                            ...
                            return new Proxy(result, safeProxyHandler);
                        }
                        return result;
                    };
                ...
                if (val && typeof val === 'object')
                    return new Proxy(val, safeProxyHandler);
                return val;
            },
            getPrototypeOf() {
                return null;
        ...
        function makeSafeFunction(fn) {
            ...
            shadowBlockedProperties(wrapped);
            return wrapped;
        }
        /**
        ...
        const rawSandbox = {
            tools: safeTools,
            console: console_,
            setTimeout: makeSafeFunction(setTimeout),
            clearTimeout: makeSafeFunction(clearTimeout),
            Promise: safePromise,
        };
        ...
        const context = vm_1.default.createContext(sandbox, { name: 'script-sandbox' });
        const result = vm_1.default.runInNewContext(script, context, {
            timeout: 120000,
        });
...
exports.ExecuteScriptTool = ExecuteScriptTool;
