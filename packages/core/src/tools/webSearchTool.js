...
const httpClient_1 = require("./_utils/httpClient");
...
class WebSearchTool {
    ...
    async execute(args) {
        var _a, _b;
        const query = String((_a = args.query) !== null && _a !== void 0 ? _a : '');
        const numResults = Math.min(Number((_b = args.numResults) !== null && _b !== void 0 ? _b : 5), 10);
        if (!query)
            return 'Error: query is required';
        let results = await this.tryBing(query, numResults);
        if (!results)
            results = await this.tryDuckDuckGo(query, numResults);
        if (!results)
            results = await this.tryGoogle(query, numResults);
        if (!results || results.length === 0) {
            return `No results found for "${query}". Try a different query.`;
        }
        const filtered = results.filter((r) => (0, urlSafety_1.isUrlSafe)(r.url).safe);
    ...
    async tryDuckDuckGo(query, maxResults) {
        try {
            const url = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
            const { body: html, status } = await (0, httpClient_1.safeFetch)(url, {
                headers: {
            ...
            if (html.includes('anomaly-modal') ||
                ...
                (0, logging_1.getGlobalLogger)().warn('WebSearchTool', 'DuckDuckGo returned CAPTCHA, skipping');
                return null;
            }
            ...
            (0, logging_1.getGlobalLogger)().warn('WebSearchTool', 'DuckDuckGo search failed', {
    ...
    async tryBing(query, maxResults) {
        try {
            ...
            (0, logging_1.getGlobalLogger)().warn('WebSearchTool', 'Bing search failed', {
                error: e === null || e === void 0 ? void 0 : e.message,
                query,
            });
            return null;
    ...
    async tryGoogle(query, maxResults) {
        try {
            ...
            if (html.includes('captcha') ||
                ...
                (0, logging_1.getGlobalLogger)().warn('WebSearchTool', 'Google returned CAPTCHA, skipping');
                return null;
            }
            ...
            (0, logging_1.getGlobalLogger)().warn('WebSearchTool', 'Google search failed', {
                error: e === null || e === void 0 ? void 0 : e.message,
                query,
            });
            return null;
    ...
    parseGoogle(html, maxResults) {
        ...
        while ((match = linkRegex.exec(html)) !== null && urls.length < maxResults) {
            ...
            if (rawUrl.includes('google.com') || rawUrl.includes('youtube.com/results'))
                continue;
            urls.push(rawUrl);
    ...
    parseDuckDuckGo(html, maxResults) {
        const results = [];
        ...
        if (results.length === 0) {
            ...
            while ((m = anyLinkRegex.exec(html)) !== null && results.length < maxResults) {
                ...
                if (text)
                    results.push({ title: text, url: m[1], snippet: '' });
            }
        }
        return results;
    }
    parseBing(html, maxResults) {
        var _a;
        const results = [];
        const blocks = [];
        let idx = 0;
        ...
        for (const block of blocks) {
            ...
            const snipMatch = (_a = block.match(/<p[^>]*class="b_lineclamp[^"]*"[^>]*>([\s\S]*?)<\/p>/)) !== null && _a !== void 0 ? _a : block.match(/<p[^>]*>([\s\S]*?)<\/p>/);
...
class WebFetchTool {
    ...
    async execute(args) {
        ...
        const maxChars = Math.min(Number((_b = args.maxChars) !== null && _b !== void 0 ? _b : 5000), 20000);
        if (!url)
            return 'Error: url is required';
        const safety = (0, urlSafety_1.isUrlSafe)(url);
        if (!safety.safe)
            return `Blocked: ${url} (${safety.reason})`;
        try {
            const { body: html, truncated } = await (0, httpClient_1.safeFetch)(url, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (compatible; CommanderBot; +https://github.com/sampan/commander)',
                },
            });
            const text = html
            ...
            if (err instanceof httpClient_1.SafeFetchError) {
