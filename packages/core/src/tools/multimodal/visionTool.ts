  ...
  async execute(args: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      if (source.startsWith('data:')) {
        ...
        if (buffer.length > maxSize) return `Error: Image too large (${(buffer.length / 1024 / 1024).toFixed(1)}MB). Max: 20MB.`;
        ...
        messages: [
          {
            ...
            content: [
              ...
              { type: 'image_url', image_url: { url: `data:${mediaType};base64,${imageData}`, detail } },
            ],
          },
        ],
        max_tokens: 4096,
      ...
      if (!apiKey) return 'Error: No API key configured. Set VISION_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY.';
