...
export class HallucinationDetector {
  ...
  constructor(options?: { knowledgeCutOffDate?: Date }) {
    this.knowledgeCutOffDate = options?.knowledgeCutOffDate ?? new Date('2026-01-01');
  }
  ...
  private detectOverconfidence(text: string, hasHedging: boolean): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
  ...
  private detectUnsupportedSpecificity(text: string, hasHedging: boolean): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
   ...
   */
  private checkEntailment(input: string, output: string): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
  ...
  private detectContradictions(text: string): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
  ...
  private detectConfidenceInconsistency(text: string): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
   ...
   */
  private detectHedgedAsFact(text: string): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
   ...
   */
  private detectEntityHallucination(text: string): HallucinationSignal[] {
    const signals: HallucinationSignal[] = [];
