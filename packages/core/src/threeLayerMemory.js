...
export class ThreeLayerMemory {
    ...
    config;
    embeddingFn = null;
    embedStore = new InMemoryEmbeddingStore();
    persistPath = null;
    ...
    load() {
        ...
        try {
            ...
            getGlobalLogger().info('ThreeLayerMemory', `Loaded ${data.entries.length} entries from ${this.persistPath}`);
            return data.entries.length;
        }
        catch (e) {
    ...
    add(content, layer, context = '', importance = 0.5, tags = [], metadata = {}) {
        ...
        if (this.memoryStore && layer !== 'working') {
            ...
            this.memoryStore.write(opts).catch((err) => {
        ...
        if (this.embeddingFn && layer !== 'working') {
            ...
            if (result instanceof Promise) {
                result
                    ...
                    .catch((e) => getGlobalLogger().debug('ThreeLayerMemory', 'embedding error', {
                    error: e?.message,
                }));
            }
            else {
    ...
    evictIfNeeded(layer) {
        ...
        const routeOutDelete = (id) => {
            if (this.memoryStore && layer !== 'working') {
                this.memoryStore.delete(id, 'default').catch((err) => {
...
export function getGlobalThreeLayerMemory() {
    return memorySingleton.get();
}
/** Reset the global memory singleton (for test isolation) */
export function resetGlobalThreeLayerMemory() {
    memorySingleton.reset();
}
export function createThreeLayerMemory(config) {
 ...
 */
export function createPersistedThreeLayerMemory(basePath) {
 ...
 */
export function mapMemoryEntryToWriteOptions(entry, projectId = 'default') {
    const kind = entry.layer === 'longterm'
 ...
 */
export function wireGlobalThreeLayerMemory(store) {
