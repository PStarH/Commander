...
exports.ThreeLayerMemory = void 0;
exports.getGlobalThreeLayerMemory = getGlobalThreeLayerMemory;
exports.resetGlobalThreeLayerMemory = resetGlobalThreeLayerMemory;
exports.createThreeLayerMemory = createThreeLayerMemory;
const memoryQualityGate_js_1 = require("./memory/memoryQualityGate.js");
const thompsonMemoryScorer_js_1 = require("./memory/thompsonMemoryScorer.js");
...
// Three-Layer Memory
// ========================================
const embedding_1 = require("./runtime/embedding");
const logging_1 = require("./logging");
class ThreeLayerMemory {
    constructor(config) {
        this.memories = new Map();
        this.embeddingFn = null;
        this.embedStore = new embedding_1.InMemoryEmbeddingStore();
        this.config = { ...DEFAULT_CONFIG, ...config };
        this.qualityGate = new memoryQualityGate_js_1.MemoryQualityGate();
        this.thompsonScorer = new thompsonMemoryScorer_js_1.ThompsonMemoryScorer();
    }
     ...
     */
    add(content, layer, context = '', importance = 0.5, tags = [], metadata = {}) {
        // Quality gate: fast path (0 tokens)
        if (layer !== 'working' && !(0, memoryQualityGate_js_1.quickQualityCheck)(content, importance)) {
            (0, logging_1.getGlobalLogger)().debug('ThreeLayerMemory', 'Memory rejected by quality gate', {
        ...
        if (layer !== 'working' && content.length > 20) {
            try {
                ...
                if (contradictionIds.length > 0) {
                    ...
                    (0, logging_1.getGlobalLogger)().debug('ThreeLayerMemory', 'Contradiction detected in memory write', {
                ...
                (0, logging_1.getGlobalLogger)().debug('ThreeLayerMemory', 'Contradiction check failed (best-effort)', {
                    error: e === null || e === void 0 ? void 0 : e.message,
                });
            }
        }
        const entryMetadata = contradictionIds.length > 0 ? { ...metadata, contradictions: contradictionIds } : metadata;
        const entry = {
            id: generateUUID(),
            layer,
            content,
        ...
        if (this.embeddingFn && layer !== 'working') {
            ...
            if (result instanceof Promise) {
                result
                    ...
                    .catch((e) => (0, logging_1.getGlobalLogger)().debug('ThreeLayerMemory', 'embedding error', {
                    error: e === null || e === void 0 ? void 0 : e.message,
                }));
            }
            else {
    ...
    updateAccess(entry) {
        ...
        if (entry.layer === 'episodic') {
            ...
            entry.decayScore = Math.max(0, entry.decayScore - config.decayRate * (1 - entry.importance * config.importanceBoost));
        }
        // Procedural 层应用衰减
    ...
    query(query) {
        ...
        // 按上下文过滤
        if (query.context) {
            results = results.filter((m) => m.context.toLowerCase().includes(query.context.toLowerCase()));
        }
        // 按重要性过滤
        ...
        results.sort((a, b) => {
            const scoreA = (0, embedding_1.calculateMemoryScore)(a, queryEmbedding, this.embedStore.getEmbedding(a.id));
            const scoreB = (0, embedding_1.calculateMemoryScore)(b, queryEmbedding, this.embedStore.getEmbedding(b.id));
     ...
     */
    getWorkingContext(maxEntries = 10) {
     ...
     * 归档到情景层
     */
    archiveToEpisodic(id) {
        const entry = this.memories.get(id);
        if (!entry || entry.layer !== 'working')
            return false;
     ...
     * 应用时间衰减 (定时调用)
     */
    applyTimeDecay(hoursElapsed) {
        const toDelete = [];
        for (const entry of this.memories.values()) {
            if (entry.layer === 'episodic' || entry.layer === 'procedural') {
                const config = this.config[entry.layer];
                const decay = config.baseDecayPerHour * hoursElapsed * (1 - entry.importance * config.importanceBoost);
        ...
        if (this.embeddingFn) {
            ...
            if (!(queryEmb instanceof Promise)) {
                ...
                for (const entry of this.memories.values()) {
                    ...
                    const embScore = entryEmb ? (0, embedding_1.calculateMemoryScore)(entry, queryEmb, entryEmb) : 0;
     ...
     */
    updateMemoryUsefulness(memoryId, wasUseful) {
...
exports.ThreeLayerMemory = ThreeLayerMemory;
// ========================================
// Factory
// ========================================
const tenantAwareSingleton_1 = require("./runtime/tenantAwareSingleton");
const memorySingleton = (0, tenantAwareSingleton_1.createTenantAwareSingleton)(() => new ThreeLayerMemory());
function getGlobalThreeLayerMemory() {
    return memorySingleton.get();
}
/** Reset the global memory singleton (for test isolation) */
function resetGlobalThreeLayerMemory() {
    memorySingleton.reset();
}
function createThreeLayerMemory(config) {
