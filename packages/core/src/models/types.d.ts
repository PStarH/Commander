...
export type CommanderInvocationDisposition = 'ALLOW_EXECUTION' | 'REQUIRE_APPROVAL' | 'PROPOSE_ONLY' | 'DENY';
/** Atomic operations that an agent can perform within the system. */
export type CommanderOperation = 'READ_CONTEXT' | 'WRITE_LOG' | 'UPDATE_MISSION_STATUS' | 'UPDATE_MISSION_FIELDS' | 'WRITE_MEMORY' | 'UPDATE_AGENT_STATE' | 'REQUEST_APPROVAL';
...
export interface SlimSnapshot {
    ...
    battleMetrics: Pick<ProjectBattleReport, 'health' | 'runningMissionCount' | 'blockedMissionCount' | 'completedMissionCount' | 'highRiskMissionCount' | 'manualGovernanceMissionCount' | 'logVolume24h' | 'completionRate'>;
    /** Most recent logs. */
...
export type MultiAgentStrategyKind = 'SINGLE_AGENT' | 'GUARDED_EXECUTION' | 'SENATE_REVIEW' | 'MANUAL_APPROVAL_GATE' | 'FANOUT_PLAN';
