...
exports.createSlimSnapshot = createSlimSnapshot;
exports.createSeedWarRoomData = createSeedWarRoomData;
exports.generateProjectBattleReport = generateProjectBattleReport;
exports.getProjectWarRoomSnapshot = getProjectWarRoomSnapshot;
const missionPriorityWeight = {
    CRITICAL: 4,
    HIGH: 3,
    MEDIUM: 2,
    LOW: 1,
};
const missionStatusWeight = {
    BLOCKED: 4,
    RUNNING: 3,
    PLANNED: 2,
    DONE: 1,
 ...
 */
function createSlimSnapshot(snapshot, options) {
    var _a, _b;
    const maxMissionsPerBucket = (_a = options.maxMissionsPerBucket) !== null && _a !== void 0 ? _a : 6;
    const maxLogs = (_b = options.maxLogs) !== null && _b !== void 0 ? _b : 8;
    const toMissionCard = (m) => ({
        id: m.id,
        title: m.title,
...
function createSeedWarRoomData(now = new Date()) {
    ...
    const project = {
        ...
        objective: 'Ship the first AI teammate command center where humans direct agents, watch execution in real time, and publish battle reports.',
        status: 'ACTIVE',
        createdAt,
        updatedAt: createdAt,
    };
...
function getProjectHealth(blockedMissionCount, runningMissionCount) {
...
function buildBattleNarrative(input) {
    const { project, totalAgents, activeAgents, totalMissions, runningMissionCount, blockedMissionCount, completedMissionCount, completionRate, logVolume24h, highRiskMissionCount, manualGovernanceMissionCount, topAgents, } = input;
    ...
    const governanceLine = highRiskMissionCount > 0
...
function generateProjectBattleReport(project, agents, missions, logs, now = new Date()) {
...
function getProjectWarRoomSnapshot(data, projectId, now = new Date()) {
    ...
    const agentWorkloads = projectAgents
        .map((agent) => {
        var _a;
        const assignedMissions = projectMissions.filter((mission) => mission.assignedAgentId === agent.id);
        const completedMissionCount = assignedMissions.filter((mission) => mission.status === 'DONE').length;
        ...
        return {
            ...
            activeMissionCount,
            completedMissionCount,
            latestLogAt: (_a = projectLogs.find((log) => log.agentId === agent.id)) === null || _a === void 0 ? void 0 : _a.createdAt,
        };
    })
        .sort((left, right) => {
        return (right.activeMissionCount - left.activeMissionCount ||
...
function sortMissions(left, right) {
    return (missionStatusWeight[right.status] - missionStatusWeight[left.status] ||
