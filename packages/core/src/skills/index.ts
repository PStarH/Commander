...
export function createSkill(
  ...
  getManager().create(name, prompt, {
    ...
    createdAt: now,
    updatedAt: now,
  }).catch(e => getGlobalLogger().warn('Skills', 'Failed to create skill', { error: (e as Error)?.message }));
...
export function listSkills(): LegacySkillDef[] {
  try {
    ...
    return fs.readdirSync(LEGACY_SKILLS_DIR)
      .filter(f => f.endsWith('.json'))
      .map(f => {
        try {
          return JSON.parse(fs.readFileSync(path.join(LEGACY_SKILLS_DIR, f), 'utf-8')) as LegacySkillDef;
        } catch { return null; }
      })
...
export function recordSkillUsage(name: string): void {
  getManager().recordUsage(name, true).catch(e =>
...
export function deleteSkill(name: string): boolean {
  getManager().delete(name).catch(e =>
