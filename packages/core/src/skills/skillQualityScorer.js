...
exports.buildRubricPrompt = buildRubricPrompt;
exports.evaluateWithRubric = evaluateWithRubric;
...
const DEFAULT_RUBRIC_CRITERIA = [
    ...
    {
        name: 'correctness',
        description: 'Does the approach described appear technically correct and follow best practices?',
        score: 0,
        weight: 0.25,
    },
    {
 ...
 */
function buildRubricPrompt(skill, criteria) {
    return [
 ...
 */
async function evaluateWithRubric(skill, config) {
    ...
    if (!rawScores || rawScores.length !== criteria.length)
        return null;
    if (rawScores.some((s) => typeof s !== 'number' || s < 0 || s > 1))
        return null;
 ...
 */
function computeDeterministicScore(skill) {
    const factors = [
 ...
 */
async function computeQualityScore(skill, llmConfig) {
    var _a;
    ...
    const llmWeight = (_a = llmConfig.llmWeight) !== null && _a !== void 0 ? _a : 0.4;
...
function scoreUsage(usageCount) {
    if (usageCount >= 50)
        return 1.0;
    if (usageCount >= 20)
        return 0.8;
