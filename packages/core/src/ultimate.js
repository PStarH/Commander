...
exports.shouldDecompose = exports.measureTaskComplexity = exports.QualityGateExecutor = exports.TokenBudgetAllocator = exports.DEFAULT_MODEL_CONFIG = exports.AdaptiveOrchestrator = void 0;
const index_1 = require("./index");
Object.defineProperty(exports, "measureTaskComplexity", { enumerable: true, get: function () { return index_1.measureTaskComplexity; } });
Object.defineProperty(exports, "shouldDecompose", { enumerable: true, get: function () { return index_1.shouldDecompose; } });
...
class AdaptiveOrchestrator {
     ...
     * 分析任务并选择最优编排模式
     */
    analyze(task, allTasks) {
        const complexity = (0, index_1.measureTaskComplexity)(task, allTasks, this.options);
        const reasoning = [];
        // 根据复杂度选择模式
    ...
    selectMode(complexity, task, reasoning) {
    ...
    allocateTokens(mode, complexity) {
        // 基础预算分配
        const baseBudget = {
            leadAgent: 0.4, // 大模型决策
    ...
    setupQualityGates(mode, complexity) {
        const gates = [];
        // 所有模式都需要基础验证
        gates.push({
    ...
    estimateDuration(mode, complexity) {
        // 基础时间估算（毫秒）
...
class TokenBudgetAllocator {
    constructor(totalBudget = 100000, // 默认 100k tokens
    config = exports.DEFAULT_MODEL_CONFIG) {
    ...
    allocate(allocation) {
        ...
        // 验证不超过模型限制
        const validatedLead = Math.min(Math.max(leadTokens, this.config.leadModel.minTokens), this.config.leadModel.maxTokens);
        const validatedSpecialist = Math.min(Math.max(specialistTokens, this.config.specialistModel.minTokens), this.config.specialistModel.maxTokens);
        // 计算成本
     ...
     * 获取推荐的总预算
     */
    getRecommendedBudget(complexity) {
...
class QualityGateExecutor {
     ...
     * 执行质量门控检查
     */
    async execute(gates, input, output) {
        const results = [];
    ...
    async executeGate(gate, input, output) {
        switch (gate.name) {
    ...
    checkHallucination(gate, input, output) {
        const detector = (0, hallucinationDetector_1.getHallucinationDetector)();
        const inputStr = typeof input === 'string' ? input : JSON.stringify(input !== null && input !== void 0 ? input : '');
        const outputStr = typeof output === 'string' ? output : JSON.stringify(output !== null && output !== void 0 ? output : '');
    ...
    consensusVote(gate, output) {
        var _a, _b, _c, _d, _e;
        const minJudges = (_b = (_a = gate.config) === null || _a === void 0 ? void 0 : _a.minJudges) !== null && _b !== void 0 ? _b : 3;
        const threshold = (_d = (_c = gate.config) === null || _c === void 0 ? void 0 : _c.agreementThreshold) !== null && _d !== void 0 ? _d : 0.67;
        const outputStr = typeof output === 'string' ? output : JSON.stringify(output !== null && output !== void 0 ? output : '');
        const signals = [];
        let score = 1.0;
        ...
        if (hasHedging)
            signals.push('Contains hedging language (calibrated)');
        // Check for structured output
        const hasStructure = /\n\s*[-•*]\s|\n\s*\d+[.)]\s|#{1,3}\s/.test(outputStr);
        if (hasStructure)
            signals.push('Structured output detected');
        // Check for self-contradiction indicators
        const contradictions = ((_e = outputStr.match(/\bhowever\b|\bbut\b|\bon the other hand\b|\bcontrary to\b/gi)) !== null && _e !== void 0 ? _e : []).length;
        if (contradictions > 3) {
            score -= 0.2;
        ...
        const sentences = outputStr.split(/[.!?]+/).filter((s) => s.trim().length > 20);
    ...
    verifyHandoff(gate, input, output) {
        const signals = [];
        let passed = true;
        // Basic context check
        ...
        if (passed) {
            ...
            const inputWords = new Set(inputStr
                .toLowerCase()
                .split(/\s+/)
                .filter((w) => w.length > 4));
