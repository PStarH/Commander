...
export class SupplyChainAttestor {
  ...
  generateSbom(
    ...
    const document: SpdxDocument = {
      ...
      creationInfo: {
        ...
        created: now,
        comment: options?.documentComment ?? `SBOM for ${projectName} generated by Commander SupplyChainAttestor`,
      },
      packages,
      relationships,
    };
  ...
  generateProjectSbom(): SpdxDocument {
    ...
    try {
      ...
      if (fs.existsSync(commanderJsonPath)) {
        ...
        const models = config.models ?? config.defaultModel
   ...
   */
  attest(sbom: SpdxDocument, options?: {
    predicateType?: string;
    predicate?: Record<string, unknown>;
    ...
    if (expectedIdentity && bundle.verificationMaterial.certificate) {
      try {
        ...
        if (!certPem.includes(expectedIdentity)) {
          warnings.push(
            `Expected identity "${expectedIdentity}" not found in certificate`,
          );
        }
      } catch {
        /* soft-check */
...
const attestorSingleton = createTenantAwareSingleton(
  () => new SupplyChainAttestor(),
);

export function getSupplyChainAttestor(
  config?: Partial<AttestorConfig>,
): SupplyChainAttestor {
