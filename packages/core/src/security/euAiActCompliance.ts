...
export class EuAiActComplianceReporter {
  ...
  formatMarkdown(report: EuAiActReport): string {
    const lines: string[] = [
      ...
      report.meta.previousReportId ? `**Previous Report:** ${report.meta.previousReportId}` : '',
      '',
      '---',
      '',
      '## Compliance Summary',
      ...
      '',
      '### Human-in-the-Loop Mechanisms',
      ...report.article13.hitlMechanisms.map(
        (m) =>
      ...
      ...report.article14.highRiskCategories.map(
        (r) =>
  ...
  compareReports(
    reportA: EuAiActReport,
    reportB: EuAiActReport,
  ): {
    scoreDelta: number;
    ...
    const scoreDelta =
      reportB.complianceSummary.overallComplianceScore -
      reportA.complianceSummary.overallComplianceScore;

    const articleDeltas = {
      article12:
        reportB.complianceSummary.articleScores.article12 -
        reportA.complianceSummary.articleScores.article12,
      article13:
        reportB.complianceSummary.articleScores.article13 -
        reportA.complianceSummary.articleScores.article13,
      article14:
        reportB.complianceSummary.articleScores.article14 -
      ...
      hitlMechanisms: [
        {
          ...
          type: 'approval',
          description:
            '5-mode approval system (suggest, auto-edit, full-auto, read-only, plan) with per-category configuration for tool execution, file writes, and network access.',
          whenActivated:
            'Before any high-risk tool execution (file writes, network calls, shell commands)',
        ...
        },
        {
          ...
          type: 'override',
          description:
            'Approval system allows operators to deny, modify, or redirect any tool execution. ExecPolicyEngine provides programmatic override rules.',
        ...
        },
        {
          ...
          type: 'intervention',
          description:
            'Operators can terminate any agent session at any point. Circuit breakers auto-terminate on anomaly detection with configurable thresholds.',
        ...
        },
        {
          ...
          type: 'review',
          description:
            'SecurityMonitor generates alerts for anomalous behavior. Agent-SOC classifies incidents P0-P4 with defined response SLAs.',
      ...
      highRiskCategories: [
        {
          ...
          likelihood: 0.6,
          impact:
            'Agent may execute unintended actions or reveal sensitive data if injection succeeds',
          mitigationMeasures: [
        ...
        },
        {
          ...
          likelihood: 0.5,
          impact:
            'Unauthorized system access, data modification, or resource exhaustion through tool misuse',
          mitigationMeasures: [
        ...
        },
        {
          ...
          likelihood: 0.4,
          impact:
            'Corrupted agent memory could lead to persistent incorrect behavior or data leakage',
          mitigationMeasures: [
        ...
        },
        {
          ...
          likelihood: 0.35,
          impact:
            'Compromised skill, tool, or dependency could execute malicious code with agent permissions',
          mitigationMeasures: [
        ...
        },
        {
          ...
          likelihood: 0.5,
          impact:
            'Attackers could cause significant financial damage through token floods, tool loops, or concurrent bursts',
          mitigationMeasures: [
        ...
        },
        {
          ...
          likelihood: 0.7,
          impact:
            'Incorrect or fabricated information in agent outputs could lead to bad decisions',
          mitigationMeasures: [
  ...
  private scoreArticle14(article: Article14Report): number {
    ...
    const allHaveMitigations = article.highRiskCategories.every(
      (r) => r.mitigationMeasures.length > 0,
    );
    if (allHaveMitigations) score += 10;
    // Check no unacceptable residual risk
    const noUnacceptable = article.highRiskCategories.every(
      (r) => r.residualRisk !== 'unacceptable',
    );
