 ...
 */
export function _resetSilentFailureCacheForTesting(): void {
  dedupeCache.clear();
}

/** Test-only: override the dedup window. */
export function _setSilentFailureWindowForTesting(ms: number): void {
  dedupeWindowMs = ms;
}
 ...
 * replaces it.
 */
export async function withSilentFailureAsync<T>(
  fn: () => T | Promise<T>,
  context: string,
  fallback?: T,
): Promise<T | undefined> {
