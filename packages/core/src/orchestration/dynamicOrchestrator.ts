...
export class DynamicOrchestrator {
  ...
  planNextStep(context: ExecutionContext, availableTools: string[]): TaskStep | null {
    ...
    if (
      !usedTools.has('web_search') &&
      (goal.includes('search') ||
        goal.includes('find') ||
        goal.includes('latest') ||
        goal.includes('current') ||
        goal.includes('research'))
    ) {
      return {
        id: `step-${context.stepHistory.length + 1}`,
    ...
    if (!usedTools.has('web_fetch') && usedTools.has('web_search')) {
      return {
        id: `step-${context.stepHistory.length + 1}`,
        goal: 'Fetch details from search results',
        requiredTools: ['web_fetch'],
        status: 'pending',
        dependsOn: [
          context.stepHistory.find((s) => s.requiredTools.includes('web_search'))?.id || '',
        ],
        estimatedDifficulty: 'easy',
    ...
    if (
      (goal.includes('image') ||
        goal.includes('screenshot') ||
        goal.includes('diagram') ||
        goal.includes('visual')) &&
      !usedTools.has('vision_analyze')
    ) {
      return {
        id: `step-${context.stepHistory.length + 1}`,
        goal: 'Analyze visual information',
    ...
    if (
      ...
      goal.includes('function') ||
      goal.includes('script') ||
      goal.includes('python')
    ) {
      if (!usedTools.has('code_search')) {
        return {
          id: `step-${context.stepHistory.length + 1}`,
          goal: 'Search existing code for reference',
          requiredTools: ['code_search'],
          status: 'pending',
          dependsOn: [],
          estimatedDifficulty: 'easy',
      ...
      if (!usedTools.has('refine_code')) {
        return {
          id: `step-${context.stepHistory.length + 1}`,
          goal: 'Implement and verify code',
          requiredTools: ['refine_code'],
          status: 'pending',
          dependsOn: [],
    ...
    if (
      (goal.includes('calculate') || goal.includes('compute') || goal.includes('math')) &&
      !usedTools.has('python_execute')
    ) {
      return {
        id: `step-${context.stepHistory.length + 1}`,
        goal: 'Perform computation',
    ...
    if (!usedTools.has('verify_answer') && context.stepHistory.length > 2) {
      return {
        id: `step-${context.stepHistory.length + 1}`,
        goal: 'Verify answer format',
        requiredTools: ['verify_answer'],
        status: 'pending',
        dependsOn: [],
        estimatedDifficulty: 'easy',
        fallbackStrategy: 'simplify',
  ...
  private getRecoveryStep(
    ...
    if (retryCount < this.maxRetriesPerStep && failedStep.fallbackStrategy === 'retry') {
      return {
        ...failedStep,
        id: `${failedStep.id}-retry-${retryCount + 1}`,
        status: 'pending',
        error: undefined,
      };
    }
    ...
    if (failedStep.fallbackStrategy === 'alternative_tool') {
      ...
      if (availableAlt) {
        return {
          ...
          goal: `${failedStep.goal} (alternative approach)`,
          requiredTools: [availableAlt],
          status: 'pending',
          dependsOn: [],
          estimatedDifficulty: 'medium',
          fallbackStrategy: 'simplify',
