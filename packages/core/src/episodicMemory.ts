...
export class InMemoryMemoryStore implements MemoryStore {
  ...
  async update(options: MemoryManageOptions): Promise<EpisodicMemoryItem | null> {
    ...
    if (options.updates) {
      ...
      // Only bump lastAccessedAt if the caller did not explicitly set it.
      // This allows TTL curators and tests to backdate access timestamps.
      if (!('lastAccessedAt' in safeUpdates)) {
