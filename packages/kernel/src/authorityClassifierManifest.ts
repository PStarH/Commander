...
function verifyRow(value: unknown): AuthorityClassifierRow {
  ...
  assert(
    hasExactKeys(value, [
      'signature', 'category', 'owner', 'executableRoles', 'allowedSessionUsers',
      'allowedRelations', 'appResolverRequired', 'triggerBinding',
    ]),
    'row keys must exactly match the v1 contract',
  );
  assert(typeof value.signature === 'string' && value.signature.length > 0 && !value.signature.includes('*'), 'row signature must be exact');
  assert(typeof value.category === 'string' && categories.has(value.category), 'row category is not recognized');
  assert(typeof value.owner === 'string' && value.owner.length > 0 && !value.owner.includes('*'), 'row owner must be exact');
  assert(isExactStringList(value.executableRoles), 'row executableRoles must be an exact string list');
  assert(isExactStringList(value.allowedSessionUsers), 'row allowedSessionUsers must be an exact string list');
  assert(isExactStringList(value.allowedRelations), 'row allowedRelations must be an exact string list');
  ...
  if (value.triggerBinding !== null) {
    ...
    assert(hasExactKeys(value.triggerBinding, ['relation', 'events', 'columns']), 'triggerBinding keys must exactly match the v1 contract');
    assert(typeof value.triggerBinding.relation === 'string' && value.triggerBinding.relation.length > 0 && !value.triggerBinding.relation.includes('*'), 'trigger relation must be exact');
    assert(isExactStringList(value.triggerBinding.events), 'trigger events must be an exact string list');
    assert(isExactStringList(value.triggerBinding.columns), 'trigger columns must be an exact string list');
  }
...
function verifyPolicy(value: unknown): AuthorityClassifierPolicy {
  ...
  assert(hasExactKeys(value, ['relation', 'role', 'name', 'command']), 'policy keys must exactly match the v1 contract');
  for (const field of ['relation', 'role', 'name', 'command'] as const) {
    assert(typeof value[field] === 'string' && value[field].length > 0 && !value[field].includes('*'), `policy ${field} must be exact`);
  }
...
export function verifyAuthorityClassifierManifest(value: unknown): AuthorityClassifierManifestV1 {
  ...
  assert(hasExactKeys(value, ['version', 'rows', 'policies']), 'manifest keys must exactly match the v1 contract');
  ...
  assert(new Set(rows.map((row) => row.signature)).size === rows.length, 'manifest has duplicate signatures');
  assert(new Set(policies.map((policy) => `${policy.relation}:${policy.role}:${policy.name}:${policy.command}`)).size === policies.length, 'manifest has duplicate policies');
...
export function verifyAuthorityClassifierCatalog(
  ...
  for (const [signature, expectedRow] of expectedRows) {
    ...
    if (!sameStringList(observedRow.allowedRelations, expectedRow.allowedRelations)) catalogMismatch('dependency');
    if (!sameStringList(observedRow.executableRoles, expectedRow.executableRoles)
      || !sameStringList(observedRow.allowedSessionUsers, expectedRow.allowedSessionUsers)) {
      catalogMismatch('grant');
    }
    if (observedRow.owner !== expectedRow.owner || observedRow.appResolverRequired !== expectedRow.appResolverRequired) {
      catalogMismatch('row');
    }
    if (!sameTriggerBinding(observedRow.triggerBinding, expectedRow.triggerBinding)) catalogMismatch('trigger');
  }
...
export function exportAuthorityClassifierManifest(manifest: AuthorityClassifierManifestV1 = AUTHORITY_CLASSIFIER_MANIFEST_V1): string {
  return canonicalBootstrapJson({
    version: manifest.version,
    rows: [...manifest.rows].sort((a, b) => a.signature.localeCompare(b.signature)),
    policies: [...manifest.policies].sort((a, b) => `${a.relation}:${a.role}:${a.name}:${a.command}`.localeCompare(`${b.relation}:${b.role}:${b.name}:${b.command}`)),
  });
}

export function authorityClassifierManifestSha256(manifest: AuthorityClassifierManifestV1 = AUTHORITY_CLASSIFIER_MANIFEST_V1): string {
...
const rawManifest = readFileSync(new URL('../src/authorityClassifierManifest.v1.json', import.meta.url), 'utf8');
export const AUTHORITY_CLASSIFIER_MANIFEST_V1 = deepFreeze(
  verifyAuthorityClassifierManifest(JSON.parse(rawManifest)),
);

if (authorityClassifierManifestSha256(AUTHORITY_CLASSIFIER_MANIFEST_V1)
