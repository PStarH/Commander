...
// F-K1-12: live fixture proof for the Task 1 lifecycle initializer. Without a
// PostgreSQL 16 instance the suite cannot run — report the missing prerequisite
// by name instead of skipping silently.
const LIVE_PG_SKIP_REASON =
  'NOT VERIFIED: COMMANDER_TASK1_PG_URL is unset — Task 1 lifecycle initializer ' +
  'PostgreSQL fixture proof did not run';
if (!ownerUrl) {
  process.stderr.write(`[kernel:live-postgres] ${LIVE_PG_SKIP_REASON}\n`);
}

describe(
  'Task 1 lifecycle initializer PostgreSQL fixture',
  {
