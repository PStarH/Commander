  ...
  if (typeof value === 'string') {
    try {
      ...
      process.stderr.write(
        `[kernel:postgres] JSON parse error in column ${column}: corrupted data, using fallback\n`,
      );
      return fallback;
    }
  }
...
export class PostgresKernelRepository implements KernelRepository {
  ...
  private async claimReconcileEffectsViaRpc(
    ...
    try {
      ...
      if (raw == null) return [];
      const rows = (
        typeof raw === 'string' ? safeJsonParse(raw, 'claim_reconcile_effects', []) : raw
      ) as Array<{
        effect: DbEffect;
        claimToken: string;
      }>;
  ...
  private async claimOutboxByTopicViaRpc(
    ...
    try {
      ...
      if (raw == null) return [];
      const payload = (
        typeof raw === 'string' ? safeJsonParse(raw, 'claim_outbox_by_topic', {}) : raw
      ) as {
        claimToken?: string;
