...
export class PostgresKernelRepository implements KernelRepository {
  ...
  private async appendEvent(client: SqlClient, event: Omit<KernelEvent, 'id' | 'schemaVersion' | 'occurredAt'>, outboxKey = event.runId): Promise<void> {
    const id = randomUUID();
    ...
    await client.query(`INSERT INTO commander_outbox (id,event_id,tenant_id,topic,key,payload) VALUES ($1,$2,$3,$4,$5,$6::jsonb)`, [randomUUID(), id, event.tenantId, `commander.${event.type}`, outboxKey, json({ eventId: id, type: event.type, runId: event.runId, tenantId: event.tenantId, ...event.payload })]);
  }
