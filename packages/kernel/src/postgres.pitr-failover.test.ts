...
async function initPostgres(
  ...
  await runCommand('sh', [
    '-c',
    `cat > "${join(pgdata, 'postgresql.conf')}" <<'EOF'\n${config}\nEOF`,
  ]);
  ...
  await runCommand('sh', [
    '-c',
    `cat >> "${join(pgdata, 'pg_hba.conf')}" <<'EOF'\nhost all all 127.0.0.1/32 trust\nhost replication all 127.0.0.1/32 trust\nEOF`,
  ]);
  await runCommand(join(PG_BIN, 'pg_ctl'), [
    '-D',
    pgdata,
    '-l',
    join(pgdata, 'log'),
...
async function createDatabase(port: number, name: string): Promise<void> {
  try {
    await runCommand(join(PG_BIN, 'createdb'), [
      '-h',
      '127.0.0.1',
      '-p',
      String(port),
  ...
  () => {
    it('PITR restores kernel data to a target time', async () => {
      ...
      try {
        ...
        assert.equal(
          await restoredRepo.getRun(runB.id, tenantId),
          null,
          'PITR must not restore runB',
        );
    ...
    it('failover to streaming replica keeps kernel data available', async () => {
      ...
      const primaryArchive = join(
        tmpdir(),
        `commander-failover-archive-${randomUUID().slice(0, 8)}`,
      );
      ...
      try {
        ...
        await runCommand('sh', [
          '-c',
          `cat >> "${join(standbyData, 'postgresql.conf')}" <<'EOF'\nport = ${standbyPort}\nhot_standby = on\nEOF`,
        ]);
        await runCommand(join(PG_BIN, 'pg_ctl'), [
          '-D',
          standbyData,
          '-l',
          join(standbyData, 'log'),
          'start',
          '-w',
        ]);
        ...
        assert.ok(
          await standbyRepoBefore.getRun(run.id, tenantId),
          'standby must replicate run before failover',
        );
        ...
        assert.ok(
          await promotedRepo.getRun(run.id, tenantId),
          'promoted standby must still serve run',
        );
