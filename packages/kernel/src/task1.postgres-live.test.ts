...
// F-K1-12: this suite is the live proof of Task 1 role separation and admission
// authority. It cannot run without a PostgreSQL 16 fixture, so the skip is
// named and reported loudly — absent environment is NOT_RUN, never PASS.
const LIVE_PG_SKIP_REASON =
  'NOT VERIFIED: COMMANDER_TASK1_PG_URL is unset — Task 1 real PostgreSQL role ' +
  'and admission authority proof did not run';
if (!ownerUrl) {
...
describe(
  'Task 1 real PostgreSQL role and admission authority',
  {
    skip: ownerUrl ? false : LIVE_PG_SKIP_REASON,
  },
  () => {
    ...
    it('enforces dedicated positive and negative role RPC paths', async () => {
      ...
      assert.equal(
        registrationOnly.ready,
        false,
        'registration alone must not establish readiness',
      );
      ...
      assert.deepEqual(
        zeroClaim.rows[0]?.claimed,
        [],
        'zero-claim reconciliation tick must succeed',
      );
      ...
      await assert.rejects(
        () =>
          adapterPool.query('SELECT id FROM commander_outbox WHERE tenant_id=$1 LIMIT 1', [
            tenantId,
          ]),
        /permission denied/i,
