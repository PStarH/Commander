  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const consumeResult = await consumeCompensationBatch(
          repo,
          {
            admit: async (input) => {
              ...
              if (!cmpAdmit.admitted) {
                return { admitted: false, effectId: input.effectId, replayed: false, reason: cmpAdmit.reason };
              }
              return { admitted: true, effectId: input.effectId, replayed: cmpAdmit.replayed };
            },
        ...
        await pool.query(
          'DELETE FROM commander_outbox_deliveries WHERE tenant_id = ANY($1::text[])',
          [[tenantA, tenantB, tenantC]],
        );
        await pool.query('DELETE FROM commander_outbox WHERE tenant_id = ANY($1::text[])', [
          [tenantA, tenantB, tenantC],
        ]);
        ...
        await pool.query(
          'DELETE FROM commander_tenant_execution_control WHERE tenant_id = ANY($1::text[])',
          [[tenantA, tenantB, tenantC]],
        );
      ...
      try {
        ...
        for (let round = 0; round < rounds; round++) {
          ...
          const compensation = events.find(
            (event) => event.type === 'kernel.compensation.requested',
          );
        ...
        if (tenants.length > 0) {
          ...
          await pool.query(
            'DELETE FROM commander_tenant_execution_usage WHERE tenant_id = ANY($1::text[])',
            [tenants],
          );
        }
        if (workers.length > 0) {
