  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        assert.ok(
          migrationRows.rows.length >= 3,
          'schema, RLS and roles migrations must be recorded',
        );
        ...
        const currentGenerationClaim = await repoA.claimNextStep({
          workerId: workerA,
          workerGeneration: 2,
          tenantIds: [tenantA],
          capabilities: ['agent'],
