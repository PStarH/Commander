...
export class InMemoryKernelRepository implements KernelRepository {
  ...
  async reclaimExpiredLeases(at = new Date(), limit = 100): Promise<KernelStep[]> {
    ...
    for (const step of [...this.steps.values()].filter((candidate) => candidate.state === 'RUNNING' && candidate.lease && Date.parse(candidate.lease.expiresAt) <= at.getTime()).slice(0, limit)) {
      ...
      step.state = retryable ? 'RETRY_WAIT' : 'FAILED'; step.version++; step.lease = undefined; step.updatedAt = at.toISOString();
      ...
      if (!retryable) this.finish(step.runId, 'kernel.recovery'); reclaimed.push(clone(step));
    }
    return reclaimed;
  }
  ...
  async failStep(request: FailStepRequest): Promise<KernelStep | null> {
    ...
    step.state = retry ? 'RETRY_WAIT' : 'FAILED'; step.error = request.error; step.scheduledAt = request.retryAt?.toISOString() ?? step.scheduledAt; step.version++; step.lease = undefined; step.updatedAt = now();
  ...
  async wakeRetryStep(stepId: string, tenantId: string, actor: string): Promise<KernelStep | null> {
    ...
    step.state = 'PENDING'; step.version++; step.lease = undefined; step.updatedAt = now();
  ...
  async claimExpiredTimers(at = new Date(), limit = 100): Promise<KernelTimer[]> {
    const expired = [...this.timers.values()]
      ...
      .slice(0, limit);
    for (const t of expired) { t.state = 'FIRED'; t.firedAt = now(); }
  ...
  async replayDlqEntry(dlqId: string): Promise<boolean> {
    ...
    const newMsg: KernelOutboxMessage = {
      id: randomUUID(), eventId: entry.eventId, topic: entry.topic, key: entry.key,
  ...
  private finish(runId: string, actor: string): void {
    ...
    if (steps.some((step) => step.state === 'FAILED')) { run.state = 'FAILED'; run.version++; run.updatedAt = now(); run.terminalAt = run.updatedAt; this.event('run', run.id, run.version, 'run.failed', run.tenantId, run.id, undefined, actor, {}); }
    else if (steps.length > 0 && steps.every((step) => ['SUCCEEDED', 'SKIPPED'].includes(step.state))) { run.state = 'SUCCEEDED'; run.version++; run.updatedAt = now(); run.terminalAt = run.updatedAt; this.event('run', run.id, run.version, 'run.succeeded', run.tenantId, run.id, undefined, actor, {}); }
  }
}
