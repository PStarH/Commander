...
describe('sqliteSchema parity', () => {
  it('defines all kernel tables mirrored from schema.ts', () => {
    const pgTables = [...KERNEL_SCHEMA_SQL.matchAll(/CREATE TABLE IF NOT EXISTS (\w+)/g)].map(
      (m) => m[1],
    );
    const sqliteTables = [
    ...
    for (const table of expected) {
      assert.ok(
        sqliteTables.includes(table) ||
          SQLITE_KERNEL_TABLES.includes(table as (typeof SQLITE_KERNEL_TABLES)[number]),
