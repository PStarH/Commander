            ...
            volumes: [
              {
                ...
                projected: {
                  defaultMode: 256,
                  sources: [
                    { serviceAccountToken: { audience, expirationSeconds: 600, path: 'token' } },
                    {
                      configMap: {
...
describe('Task 1 Kubernetes proof observer', () => {
  ...
  it('rejects identity, token, own-Pod, rollout, selector, port, image, and ownership drift', async () => {
    ...
    > = [
      ...
      [
        'overlong projected token',
        (_values, identity) => {
          identity.expiresAt = '2026-07-28T10:10:01.000Z';
        },
      ],
      [
      ...
      [
        'proof token lifetime drift',
        (values) => {
          values.proofPods.items[0].spec.volumes[0].projected.sources[0].serviceAccountToken.expirationSeconds = 601;
        },
      ],
      [
