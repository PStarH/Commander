...
function operation(): Task1LifecycleOperation {
  return {
    ...
    resultingState: 'enforced', operationKind: 'enforce', runtimePhase: 'enforce', platformKind: 'compose',
    previousBindingJcs: null, previousBindingSha256: null, requestedBindingJcs: canonicalBootstrapJson(binding),
    requestedBindingSha256: canonicalBootstrapSha256(binding), previousConfigurationJcs: null,
    previousConfigurationSha256: null, requestedConfigurationJcs: canonicalBootstrapJson({ operationAuditNonce: 'n'.repeat(43) }),
...
const relayContainer = {
  containerId: 'a'.repeat(64), projectLabel: 'commander', serviceLabel: 'api',
  imageDigest: binding.apiImageDigest, createdAt: '2026-07-28T00:00:00.000Z',
...
async function withRelayResponse(
  ...
  try {
    await run(createTask1ComposeRelayClientFromEnvironment({
...
describe('Task 1 Compose proof runtime', () => {
  it('projects only the authoritative api container from the relay', async () => {
    const client: Task1ComposeRelayClient = {
      ...
      containers: async () => [{
        containerId: 'a'.repeat(64), projectLabel: 'commander', serviceLabel: 'api',
        imageDigest: binding.apiImageDigest, createdAt: '2026-07-28T00:00:00.000Z',
        state: 'running', health: 'healthy', restartCount: 0, networkAttachmentIds: { default: 'n'.repeat(64) },
      }],
      container: async () => ({
        containerId: 'a'.repeat(64), projectLabel: 'commander', serviceLabel: 'api',
        imageDigest: binding.apiImageDigest, createdAt: '2026-07-28T00:00:00.000Z',
        state: 'running', health: 'healthy', restartCount: 0, networkAttachmentIds: { default: 'n'.repeat(64) },
        containerName: 'commander-api-1', imageId: `sha256:${digest('9')}`,
      }),
    };
  ...
  it('parses only exact canonical relay DTOs', async () => {
    ...
    await withRelayResponse(canonicalBootstrapJson([{
      ...relayContainer,
