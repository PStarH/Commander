...
export class CommanderGatewayClient {
  ...
  async simulateAction(input: ProposeActionInput): Promise<{ simulation: ActionSimulation }> {
    const response = await this.call('/v1/actions/simulate', { method: 'POST', body: JSON.stringify(input) });
    return this.body(response);
  }
  async proposeAction(input: ProposeActionInput): Promise<{ action: GovernedAction; idempotentReplay: boolean; accepted: boolean }> {
    ...
    return { action: body.action, idempotentReplay: body.idempotentReplay, accepted: response.status === 202 };
  }
  ...
  async reconcileAction(runId: string): Promise<Record<string, unknown>> {
    const response = await this.call(`/v1/actions/${encodeURIComponent(runId)}/reconcile`, { method: 'POST' });
    return this.body(response);
  }
