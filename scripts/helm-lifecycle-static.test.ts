...
describe('Helm lifecycle static contract', () => {
  it('keeps non-authoritative API file stores on the writable tmp volume', () => {
  ...
  it('serves the tenant-authority readiness proof through a private TLS mount', () => {
    ...
    assert.match(
      api,
      /startupProbe:[\s\S]*path: \/health[\s\S]*port: http[\s\S]*failureThreshold: 90[\s\S]*periodSeconds: 2/,
    );
    assert.match(
      api,
  ...
  it('runs contained tenant-cutover-prove only as an ephemeral post-rollout hook', () => {
    ...
    assert.match(
      prove,
      /serviceAccountToken:[\s\S]*audience: commander-tenant-cutover-proof\/v1[\s\S]*expirationSeconds: 600[\s\S]*path: token/,
    );
    assert.match(
      prove,
