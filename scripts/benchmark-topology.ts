 ...
 *   OPENAI_API_KEY=sk-... npx tsx scripts/benchmark-topology.ts --topology=hybrid --iterations=5
 *   OPENAI_API_KEY=sk-... npx tsx scripts/benchmark-topology.ts --topology=all --iterations=3 --output=report.json
 *
 * Exit codes:
...
const DEFAULT_DELAY_MS = 3500;         // ~17 RPM — avoids StepFun rate limits
const DEFAULT_RPM = 20;
const MAX_429_RETRIES = 3;
const SYSTEM_DEFAULT = 'You are a precise, concise engineering assistant. Respond in 2-4 sentences.';
const SYSTEM_EXPERT = 'You are a senior software architect. Provide detailed, technically precise answers.';
const SYSTEM_CREATIVE = 'You are an innovative thinker who approaches problems from unusual angles. Be creative but rigorous.';
const SYSTEM_CRITIC = 'You are a critical reviewer. Identify flaws, gaps, and improvement opportunities in the provided answer.';
...
const MODEL_PRICING = {
  ...
  'step-3.7-flash':      { input: 0.50,  output: 2.00  },
};
...
const TOPO_ORDER: TopologyName[] = ['single','sequential','parallel','hierarchical','hybrid','debate','ensemble','evaluator_optimizer','handoff','consensus'];
...
interface AgentResult extends CallResult {
  agentIndex: number; phase: string;
}

interface RunResult {
  iteration: number; task: string; topology: string;
  agents: AgentResult[];
  wallClockMs: number;
...
interface SummaryStats {
  topology: string; costMult: number; expectedLatency: string;
  totalRuns: number; successes: number; failures: number; failureRate: number;
  latencyMs: Record<string, number>;
  tokPerRun: number; tokInAvg: number; tokOutAvg: number;
  estCostUsd: number; callsPerRun: number;
}
...
async function throttle(): Promise<void> {
  const prev = _rlChain;
  let release: () => void;
  _rlChain = new Promise<void>(r => { release = r; });
  await prev;
  const now = Date.now();
...
async function callAPI(
  ...
  sys: string, user: string, signal?: AbortSignal,
): Promise<CallResult> {
  await throttle();
  ...
  for (let attempt = 0; attempt <= MAX_429_RETRIES; attempt++) {
    ...
    try {
      const res = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({ model, messages: [{ role: 'system', content: sys }, { role: 'user', content: user }], max_tokens: 512, temperature: 0.3 }),
        signal,
      });
      ...
      return { latencyMs: ms, inputTokens: d.usage?.prompt_tokens ?? 0, outputTokens: d.usage?.completion_tokens ?? 0, success: true };
    } catch (e) {
      lastErr = (e as Error).message;
      if (attempt < MAX_429_RETRIES) continue;
      return { latencyMs: Math.round(performance.now() - t0), inputTokens: 0, outputTokens: 0, success: false, error: lastErr };
    }
  }
  return { latencyMs: Math.round(performance.now() - t0), inputTokens: 0, outputTokens: 0, success: false, error: lastErr };
}
...
RUNNERS.single = async (ctx) => {
  ...
  return {
    iteration: ctx.iter, task: ctx.task, topology: 'single',
...
RUNNERS.sequential = async (ctx) => {
  ...
  return {
    iteration: ctx.iter, task: ctx.task, topology: 'sequential', agents,
...
RUNNERS.parallel = async (ctx) => {
  ...
  const results = await Promise.all(
    Array.from({ length: N }, (_, i) =>
      callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_DEFAULT, ctx.task)
        .then(r => ({ agentIndex: i + 1, phase: 'worker', ...r } as AgentResult))),
  );
  agents.push(...results);
  const ok = results.filter(r => r.success);
  if (ok.length === N) {
    const agg = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
  ...
  const wallMs = Math.max(...results.map(r => r.latencyMs)) + (agents.find(a => a.phase === 'merge')?.latencyMs ?? 0);
  return {
    iteration: ctx.iter, task: ctx.task, topology: 'parallel', agents, wallClockMs: wallMs,
...
RUNNERS.hierarchical = async (ctx) => {
  const agents: AgentResult[] = [];
  const plan = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
  ...
  const workers = await Promise.all(
    Array.from({ length: 3 }, (_, i) =>
      callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_EXPERT,
  ...
  const syn = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
...
RUNNERS.hybrid = async (ctx) => {
  ...
  for (let c = 0; c < 2; c++) {
    ...
    for (let s = 0; s < 2; s++) {
      const p = s === 0 ? `[Chain ${c + 1}] ${ctx.task}` : `[Chain ${c + 1}] Continue the analysis.`;
      const r = await callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_DEFAULT, p);
  ...
  const merge = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
...
RUNNERS.debate = async (ctx) => {
  ...
  const results = await Promise.all(
    Array.from({ length: N }, (_, i) =>
      callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_DEFAULT, ctx.task)
        .then(r => ({ agentIndex: i + 1, phase: 'debater', ...r } as AgentResult))),
  );
  agents.push(...results);
  const ok = results.filter(r => r.success);
  if (ok.length === N) {
    const judge = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
  ...
  return {
    ...
    wallClockMs: Math.max(...results.map(r => r.latencyMs)) + (agents.find(a => a.phase === 'judge')?.latencyMs ?? 0),
    success: agents.every(a => a.success),
  };
};
...
RUNNERS.ensemble = async (ctx) => {
  ...
  const results = await Promise.all(
    sysPrompts.map((sp, i) =>
      callAPI(ctx.baseUrl, ctx.key, ctx.model, sp, ctx.task)
        .then(r => ({ agentIndex: i + 1, phase: `voter${i + 1}`, ...r } as AgentResult))),
  );
  agents.push(...results);
  const ok = results.filter(r => r.success);
  if (ok.length === 3) {
    const vote = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
  ...
  return {
    ...
    wallClockMs: Math.max(...results.map(r => r.latencyMs)) + (agents.find(a => a.phase === 'vote')?.latencyMs ?? 0),
    success: agents.every(a => a.success),
  };
};
...
RUNNERS.evaluator_optimizer = async (ctx) => {
  ...
  const evalR = await callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_CRITIC,
  ...
  const ref = await callAPI(ctx.baseUrl, ctx.key, ctx.model, SYSTEM_EXPERT,
...
RUNNERS.handoff = async (ctx) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const r = await callAPI(ctx.baseUrl, ctx.key, ctx.model,
      i === 0 ? SYSTEM_DEFAULT : `You are Agent ${i + 1}. Review handoff context and continue.`, p);
...
RUNNERS.consensus = async (ctx) => {
  ...
  for (let rnd = 0; rnd < 3; rnd++) {
    const ra = await Promise.all(
      Array.from({ length: 3 }, (_, i) =>
        callAPI(ctx.baseUrl, ctx.key, ctx.model,
          rnd === 0 ? SYSTEM_DEFAULT : `Round ${rnd + 1}. Shared: ${shared.slice(0, 200)}`,
          rnd === 0 ? ctx.task : `Round ${rnd + 1}/3. Refine position toward convergence.\n${shared ? `Context: ${shared}` : ''}`)
...
function buildFail(topo: string, ctx: { iter: number; task: string }, agents: AgentResult[], lastErr?: CallResult): RunResult {
  return {
    iteration: ctx.iter, task: ctx.task, topology: topo, agents,
    wallClockMs: agents.reduce((s, a) => s + a.latencyMs, 0),
    totalInputTokens: agents.filter(a => a.success).reduce((s, a) => s + a.inputTokens, 0),
    totalOutputTokens: agents.filter(a => a.success).reduce((s, a) => s + a.outputTokens, 0),
    success: agents.every(a => a.success),
...
function buildSummary(topo: string, runs: RunResult[], model: string): SummaryStats {
  ...
  return {
    topology: topo, costMult: cfg.c, expectedLatency: cfg.exp,
    totalRuns: runs.length, successes: ok.length, failures: runs.length - ok.length,
    failureRate: runs.length > 0 ? (runs.length - ok.length) / runs.length : 0,
    latencyMs: {
      min: ls[0] ?? 0, p50: ls[Math.floor(ls.length * 0.5)] ?? 0,
      p95: ls[Math.floor(ls.length * 0.95)] ?? 0, max: ls[ls.length - 1] ?? 0,
      avg: ls.length > 0 ? Math.round(ls.reduce((a, b) => a + b, 0) / ls.length) : 0,
    },
    tokPerRun: ok.length > 0 ? Math.round(ok.reduce((s, r) => s + r.totalInputTokens + r.totalOutputTokens, 0) / ok.length) : 0,
    tokInAvg: ok.length > 0 ? Math.round(tInp / ok.length) : 0,
    tokOutAvg: ok.length > 0 ? Math.round(tOut / ok.length) : 0,
    estCostUsd: cost,
    callsPerRun: ok.length > 0 ? Math.round(ok.reduce((s, r) => s + r.agents.length, 0) / ok.length) : 0,
  };
}
...
function printReport(rpt: Report): void {
  ...
  console.log(`  Connectivity: ${rpt.connectivity.ok ? 'OK' : 'FAIL'} (${rpt.connectivity.latencyMs}ms)\n`);

  for (const [name, s] of Object.entries(rpt.summary)) {
    const icon = s.failureRate > 0.1 ? '!' : '.';
    console.log(`  ${name.toUpperCase().padEnd(22)} ${icon}  ${s.callsPerRun} calls ×${s.costMult}  ${fmtLat(s.latencyMs.avg).padEnd(10)} ${s.tokPerRun}tok/run  fail=${(s.failureRate * 100).toFixed(0)}%  $${s.estCostUsd.toFixed(4)}`);
  }

  console.log(`\n  ${'─'.repeat(56)}`);
  console.log(`  ${'RANK'.padEnd(5)} ${'TOPOLOGY'.padEnd(20)} ${'LATENCY'.padEnd(12)} ${'COST/100'.padEnd(10)} ${'CALLS'.padEnd(6)} ${'× vs S'.padEnd(8)}`);
  console.log(`  ${'─'.repeat(5)} ${'─'.repeat(20)} ${'─'.repeat(12)} ${'─'.repeat(10)} ${'─'.repeat(6)} ${'─'.repeat(8)}`);
  ...
  rpt.ranking.forEach((r, i) => {
    ...
    console.log(`  ${String(i + 1).padEnd(5)} ${r.topology.padEnd(20)} ${fmtLat(r.latencyAvg).padEnd(12)} $${r.costPer100.toFixed(2).padEnd(7)} ${String(r.callsPerRun).padEnd(6)} ${x.startsWith('-') ? x : `+${x}`}%`);
  });
  console.log();
  ...
  for (const s of Object.values(rpt.summary)) {
    ...
    console.log(`  ${s.topology.padEnd(20)} ${latStr.padEnd(26)} ${tokStr.padEnd(22)} $${(s.estCostUsd / Math.max(1, s.totalRuns)).toFixed(4).padEnd(6)} ${String(s.callsPerRun).padEnd(6)} ${s.successes}/${s.totalRuns}`);
  }
  console.log();
}
...
async function main(): Promise<number> {
  ...
  if (!apiKey) { console.error('FATAL: Missing API key. Set OPENAI_API_KEY.'); return 2; }
  const model = get('--model', DEFAULT_MODEL);
  const baseUrl = process.env.OPENAI_BASE_URL ?? 'https://api.openai.com/v1';
  const iterations = Math.max(1, Math.min(50, parseInt(get('--iterations', String(DEFAULT_ITERATIONS)), 10) || 1));
  ...
  log(`Model: ${model}  Iterations: ${iterations}  Topologies: ${topologies.length}`);
  log();

  // Warmup
  log('Warmup ...');
  const warm = await callAPI(baseUrl, apiKey, model, SYSTEM_DEFAULT, 'Reply "ok".');
  if (!warm.success) { log(`Warmup FAILED: ${warm.error}`); return 1; }
  ...
  for (const topo of topologies) {
    ...
    if (!runner) { log(`Skip: no runner for ${topo}`); continue; }
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      runs.push(r);
      if (r.success) {
        log(`  [${i + 1}/${iterations}] ${fmtShort(r.wallClockMs).padEnd(8)} ${r.totalInputTokens + r.totalOutputTokens}tok  ${r.agents.filter(a => a.success).length}/${r.agents.length}ok`);
      } else {
...
main().then(
  c => process.exit(c),
  e => { console.error('FATAL:', e); process.exit(2); },
);
