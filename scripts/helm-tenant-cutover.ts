...
export function commandFailureCode(
  ...
  if (command === 'create' && stdin) {
    try {
      ...
      } else if (object.kind === 'SelfSubjectReview') {
        createCode = 'TENANT_CUTOVER_KUBECTL_CREATE_SELF_SUBJECT_REVIEW_FAILED';
        forbiddenCreateCode = 'TENANT_CUTOVER_KUBECTL_CREATE_SELF_SUBJECT_REVIEW_FORBIDDEN';
